...
int main(int argc, char* argv[])
{
    ...
    sprintf(comment, "%s -by %f %s%s%s%s%s%s%s %s", argv0, factor, (largeFilter ? "-expensiveFilter " : ""),
            (isFilterFileEmpty ? "" : "-f "), (isFilterFileEmpty ? "" : filterFile), (isFilterFileEmpty ? "" : " "),
            (linearInterp ? "-linearSigInterp " : ""), (interpFilt ? "" : "-noFilterInterp "), insfname, outsfname);
    ...
    printf("\nStarting Conversion\n");
    outCountReal = resample(factor, infd, outfd, inCount, outCount, nChans, interpFilt, linearInterp, largeFilter, filterFile);
