...
/* helper functions */
int** sndlib_allocate_buffers(unsigned, unsigned);
void sndlib_free_buffers(int**, unsigned);

/* some handy macros */

#define IS_FLOAT_FORMAT(format) ((format) == snd_32_float || (format) == snd_32_float_little_endian)

#define NOT_A_SOUND_FILE(header_type) ((header_type) == MUS_UNSUPPORTED || (header_type) == MUS_RAW)

/* The header types that sndlib can write, as of sndlib-5.5. */
#define WRITEABLE_HEADER_TYPE(type) ((type) == MUS_AIFF || (type) == MUS_NEXT || (type) == MUS_RIFF || (type) == MUS_IRCAM)
