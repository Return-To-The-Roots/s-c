...
#define MUS_STANDARD_ERROR(Error_Type, Error_Message) mus_print("%s\n  [%s[%d] %s]", Error_Message, __FILE__, __LINE__, c__FUNCTION__)
...
static void sgi_mus_print(char* msg)
{
    ...
    {
        if(sgi_err_buf == NULL)
            sgi_err_buf = (char*)CALLOC(PRINT_BUFFER_SIZE, sizeof(char));
...
#define RETURN_ERROR_EXIT(Error_Type, Audio_Line, Ur_Error_Message)               \
    ...
    {                                                                             \
        char* Error_Message;                                                      \
        Error_Message = Ur_Error_Message;                                         \
        if(Audio_Line != -1)                                                      \
            al_free(Audio_Line);                                                  \
        if(Error_Message)                                                         \
        {                                                                         \
            MUS_STANDARD_ERROR(Error_Type, Error_Message);                        \
            FREE(Error_Message);                                                  \
        } else                                                                    \
            MUS_STANDARD_ERROR(Error_Type, mus_error_type_to_string(Error_Type)); \
        end_sgi_print();                                                          \
        return (MUS_ERROR);                                                       \
    } while(false)
...
static int check_queue_size(int size, int chans)
{
    if((chans == 1) && (size > MONO_QUEUE_MAX_SIZE))
        return (MONO_QUEUE_MAX_CHOICE);
    if((chans == 1) && (size < MONO_QUEUE_MIN_SIZE))
        return (MONO_QUEUE_MIN_CHOICE);
    if((chans > 1) && (size > STEREO_QUEUE_MAX_SIZE))
        return (STEREO_QUEUE_MAX_CHOICE);
    if((chans > 1) && (size < STEREO_QUEUE_MIN_SIZE))
        return (STEREO_QUEUE_MIN_CHOICE);
    return (size);
}
...
static int to_al_interface_or_device(int dev, int which)
{
    switch(dev)
    {
        ...
        case MUS_AUDIO_LINE_OUT:
            return (alGetResourceByName(AL_SYSTEM, "Line Out2", which));
            break;                                                                                              /* ?? */
...
int mus_audio_open_output(int ur_dev, int srate, int chans, int format, int requested_size)
{
    ...
    if(line == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_NO_LINES_AVAILABLE, line, "no free audio lines?");
    channels[line] = chans;
    line_out[line] = 1;
    ...
    if(requested_size == 0)
        ...
        size = check_queue_size(requested_size, chans);
    ...
    if(sgi_format == MUS_ERROR)
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, -1,
                          mus_format("format %d (%s) not supported by SGI", format, mus_audio_format_name(format)));
#ifdef AL_RESOURCE
    ...
    if(!(device[line]))
        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, -1, mus_format("device %d (%s) not available", dev, mus_audio_device_name(dev)));
#if 0
  ...
  if (device[line] == -1)
    ...
    if(alSetParams(device[line], z, 2) == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_SRATE_NOT_AVAILABLE, -1, mus_format("can't set srate of %s to %d", mus_audio_device_name(dev), srate));
#else
    ...
    if(!(config[line]))
        RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, -1, "can't allocate audio configuration?");
    if((al_setsampfmt(config[line], sgi_format) == -1)
       || (al_setwidth(config[line], width) == -1)) /* this is a no-op in the float and double cases */
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, line,
                          mus_format("audio format %d (%s, SGI: %d) not available on device %d (%s)", format, mus_audio_format_name(format),
                                     sgi_format, dev, mus_audio_device_name(dev)));
    if(al_setchannels(config[line], chans) == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, line,
                          mus_format("can't get %d channels on device %d (%s)", chans, dev, mus_audio_device_name(dev)));
    ...
    if(al_setqueuesize(config[line], size) == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_SIZE_NOT_AVAILABLE, line,
                          mus_format("can't get queue size %d on device %d (%s) (chans: %d, requested_size: %d)", size, dev,
                                     mus_audio_device_name(dev), chans, requested_size));

#ifdef AL_RESOURCE
    if(alSetDevice(config[line], device[line]) == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, line, mus_format("can't get device %d (%s)", dev, mus_audio_device_name(dev)));
#endif

    port[line] = al_openport("dac", "w", config[line]);
    if(!(port[line]))
        RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, line,
                          mus_format("can't open output port on device %d (%s)", dev, mus_audio_device_name(dev)));
    line_in_use[line] = 1;
    end_sgi_print();
    return (line);
}
...
#endif
        RETURN_ERROR_EXIT(MUS_AUDIO_WRITE_ERROR, -1, "write error");
    end_sgi_print();
    return (MUS_NO_ERROR);
}
...
int mus_audio_close(int line)
{
    ...
    if(line_in_use[line])
    {
        ...
        line_in_use[line] = 0;
        if(err)
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_CLOSE, -1, mus_format("can't close audio port %p (line %d)", port[line], line));
    }
    end_sgi_print();
    return (MUS_NO_ERROR);
}
...
int mus_audio_open_input(int ur_dev, int srate, int chans, int format, int requested_size)
{
    ...
    if(line == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_NO_LINES_AVAILABLE, -1, "no free audio lines?");
    channels[line] = chans;
    line_out[line] = 0;
    datum_size[line] = mus_bytes_per_sample(format);
#ifdef AL_RESOURCE
        ...
        if(itf)
        {
            ...
            if(alSetParams(device[line], &pv, 1) == -1)
                RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, -1,
                                  mus_format("can't set up device %d (%s)", dev, mus_audio_device_name(dev)));
        }
    }
    if(!(device[line]))
        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, -1,
                          mus_format("can't get input device %d (%s)", dev, mus_audio_device_name(dev)));
    x[0].param = AL_RATE;
    ...
    switch(dev)
    {
        ...
        default:
            RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, -1,
                              mus_format("audio input device %d (%s) not available", dev, mus_audio_device_name(dev)));
            break;
    }
    device[line] = AL_DEFAULT_DEVICE;
    ...
    if(ALsetparams(device[line], sr, 2) == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, -1,
                          mus_format("can't set up input device %d (%s)", dev, mus_audio_device_name(dev)));
    check_quad(device[line], chans);
    sr[0] = AL_INPUT_RATE;
    sr[1] = srate;
    if(ALsetparams(device[line], sr, 2) == -1)
    ...
    if(!(port[line]))
        RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, line,
                          mus_format("can't open input port on device %d (%s)", dev, mus_audio_device_name(dev)));
    line_in_use[line] = 1;
    end_sgi_print();
    return (line);
}
...
#endif
        RETURN_ERROR_EXIT(MUS_AUDIO_READ_ERROR, -1, "read error");
    end_sgi_print();
    return (MUS_NO_ERROR);
}
...
int mus_audio_mixer_read(int ur_dev, int field, int chan, float* val)
{
    ...
    if(field != MUS_AUDIO_PORT)
    {
        ...
        if(!rv)
            RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, -1,
                              mus_format("can't read %s field %d (%s)", mus_audio_device_name(dev), field, mus_audio_device_name(field)));
    }
    switch(field)
    {
        case MUS_AUDIO_PORT:
             ...
             */
            i = 0;
            if(alGetResourceByName(AL_SYSTEM, "Microphone", AL_DEVICE_TYPE) != 0)
            {
                if((i + 1) < chan)
                    val[i + 1] = MUS_AUDIO_MICROPHONE;
                i++;
            }
            if(alGetResourceByName(AL_SYSTEM, "Analog Out", AL_DEVICE_TYPE) != 0)
            {
                if((i + 1) < chan)
                    val[i + 1] = MUS_AUDIO_DAC_OUT;
                i++;
            }
            if(alGetResourceByName(AL_SYSTEM, "ADAT In", AL_DEVICE_TYPE) != 0)
            {
                if((i + 1) < chan)
                    val[i + 1] = MUS_AUDIO_ADAT_IN;
                i++;
            }
            if(alGetResourceByName(AL_SYSTEM, "AES In", AL_DEVICE_TYPE) != 0)
            {
                if((i + 1) < chan)
                    val[i + 1] = MUS_AUDIO_AES_IN;
                i++;
            }
            if(alGetResourceByName(AL_SYSTEM, "ADAT Out", AL_DEVICE_TYPE) != 0)
            {
                if((i + 1) < chan)
                    val[i + 1] = MUS_AUDIO_ADAT_OUT;
                i++;
            }
            if(alGetResourceByName(AL_SYSTEM, "AES Out", AL_DEVICE_TYPE) != 0)
            {
                if((i + 1) < chan)
                    val[i + 1] = MUS_AUDIO_AES_OUT;
                i++;
            }
            if(alGetResourceByName(AL_SYSTEM, "Line In", AL_DEVICE_TYPE) != 0)
            {
                if((i + 1) < chan)
                    val[i + 1] = MUS_AUDIO_LINE_IN;
                i++;
            }
            /* if (alGetResourceByName(AL_SYSTEM, "DAC2 In", AL_DEVICE_TYPE) != 0) {if ((i + 1) < chan) val[i + 1] = MUS_AUDIO_DIGITAL_IN;
             * i++;} */
            val[0] = i;
            break;
        ...
        case MUS_AUDIO_CHANNEL:
            ...
            if(alGetParams(rv, x, 1) == -1)
                RETURN_ERROR_EXIT(MUS_AUDIO_READ_ERROR, -1, mus_format("can't read channel setting of %s", mus_audio_device_name(dev)));
            val[0] = (float)(x[0].value.i);
            break;
        case MUS_AUDIO_SRATE:
            x[0].param = AL_RATE;
            if(alGetParams(rv, x, 1) == -1)
                RETURN_ERROR_EXIT(MUS_AUDIO_READ_ERROR, -1, mus_format("can't read srate setting of %s", mus_audio_device_name(dev)));
            val[0] = (float)(x[0].value.i);
            break;
        case MUS_AUDIO_AMP:
            if(alGetParamInfo(rv, AL_GAIN, &pinf) == -1)
                RETURN_ERROR_EXIT(MUS_AUDIO_READ_ERROR, -1, mus_format("can't read gain settings of %s", mus_audio_device_name(dev)));
            if(pinf.min.ll == pinf.max.ll)
                RETURN_ERROR_EXIT(MUS_AUDIO_AMP_NOT_AVAILABLE, -1,
                                  mus_format("%s's gain apparently can't be set", mus_audio_device_name(dev)));
            ...
            if(x[0].sizeOut <= chan)
                RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, -1,
                                  mus_format("can't read gain settings of %s channel %d", mus_audio_device_name(dev), chan));
        ...
        default:
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, -1,
                              mus_format("can't read %s setting of %s", mus_audio_device_name(field), mus_audio_device_name(dev)));
            break;
    }
    end_sgi_print();
    return (MUS_NO_ERROR);
...
int mus_audio_mixer_write(int ur_dev, int field, int chan, float* val)
{
    ...
    if(!rv)
        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, -1,
                          mus_format("can't write %s field %d (%s)", mus_audio_device_name(dev), field, mus_audio_device_name(field)));
    switch(field)
    {
        case MUS_AUDIO_SRATE:
            x[0].param = AL_RATE;
        ...
        case MUS_AUDIO_AMP:
            ...
            if(alGetParamInfo(rv, AL_GAIN, &pinf) == -1)
                RETURN_ERROR_EXIT(MUS_AUDIO_READ_ERROR, -1, mus_format("can't write gain settings of %s", mus_audio_device_name(dev)));
            ...
            if(x[0].sizeOut <= chan)
                RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, -1,
                                  mus_format("can't write gain settings of %s channel %d", mus_audio_device_name(dev), chan));
            ...
            if(alSetParams(rv, x, 1) == -1)
                RETURN_ERROR_EXIT(MUS_AUDIO_WRITE_ERROR, -1, mus_format("can't write gain settings of %s", mus_audio_device_name(dev)));
            break;
        default:
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, -1,
                              mus_format("can't write %s setting of %s", mus_audio_device_name(field), mus_audio_device_name(dev)));
            break;
    }
    end_sgi_print();
    return (MUS_NO_ERROR);
...
static void dump_resources(ALvalue* x, int rv)
{
    ...
    for(i = 0; i < rv; i++)
    {
        ...
        if(alIsSubtype(AL_DEVICE_TYPE, x[i].i))
        {
            ...
            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "\nDevice: %s (%s), srate: %d, chans: %d", dn, dl, y[3].value.i, y[2].value.i);
            pprint(audio_strbuf);
            if(pinf.min.ll != pinf.max.ll)
            {
                ALpv yy;
                ...
                for(k = 0; k < yy.sizeOut; k++)
                {
                    mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "%.2f",
                                 dB_to_normalized(alFixedToDouble(g[k]), alFixedToDouble(pinf.min.ll), alFixedToDouble(pinf.max.ll)));
                    pprint(audio_strbuf);
                    if(k < (yy.sizeOut - 1))
                        pprint(" ");
                }
            ...
            if((nres = alQueryValues(x[i].i, AL_INTERFACE, z, 16, 0, 0)) >= 0)
                ...
                mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "query failed: %s\n", alGetErrorString(oserror()));
            pprint(audio_strbuf);
        } else
        {
...
static int decode_field(int dev, int field, int chan)
{
    switch(dev)
    {
        ...
        case MUS_AUDIO_SPEAKERS:
            switch(field)
            {
                case MUS_AUDIO_AMP: return ((chan == 0) ? AL_LEFT_SPEAKER_GAIN : AL_RIGHT_SPEAKER_GAIN); break;
                case MUS_AUDIO_SRATE: return (AL_OUTPUT_RATE); break;
            }
            break;
        case MUS_AUDIO_LINE_OUT:
            switch(field)
...
int mus_audio_mixer_read(int ur_dev, int field, int chan, float* val)
{
    ...
    switch(field)
    {
        case MUS_AUDIO_CHANNEL: val[0] = 4; break;
        case MUS_AUDIO_FORMAT:
            val[0] = 1;
            if(chan > 1)
                val[1] = MUS_BSHORT;
        ...
        case MUS_AUDIO_PORT:
            ...
            val[0] = 4;
            if(chan > 1)
                val[1] = MUS_AUDIO_LINE_IN;
            if(chan > 2)
                val[2] = MUS_AUDIO_MICROPHONE;
            if(chan > 3)
                val[3] = MUS_AUDIO_DIGITAL_IN;
            if(chan > 4)
                val[4] = MUS_AUDIO_DAC_OUT;
        ...
        case MUS_AUDIO_AMP:
            ...
            if(fld != MUS_ERROR)
            {
                ...
                if((fld == AL_LEFT_SPEAKER_GAIN) || (fld == AL_RIGHT_SPEAKER_GAIN))
                    ...
                    val[0] = 1.0 - ((float)pb[1]) / ((float)MAX_VOLUME);
            } else
                err = MUS_ERROR;
            break;
        case MUS_AUDIO_SRATE:
        ...
        case MUS_AUDIO_PORT:
            if(dev == MUS_AUDIO_DEFAULT)
            {
                ...
                if(ALsetparams(AL_DEFAULT_DEVICE, pb, 4))
                    RETURN_ERROR_EXIT(MUS_AUDIO_WRITE_ERROR, -1, mus_format("can't set mode and source of %s", mus_audio_device_name(dev)));
            } else
                err = MUS_ERROR;
            break;
        case MUS_AUDIO_CHANNEL:
            if(dev == MUS_AUDIO_MICROPHONE)
            {
                ...
                if(ALsetparams(AL_DEFAULT_DEVICE, pb, 2))
                    RETURN_ERROR_EXIT(MUS_AUDIO_WRITE_ERROR, -1,
                                      mus_format("can't set microphone to be %s", (chan == 2) ? "stereo" : "mono"));
            } else
            {
                if(dev == MUS_AUDIO_DEFAULT)
                {
                    ...
                    if(ALsetparams(AL_DEFAULT_DEVICE, pb, 2))
                        RETURN_ERROR_EXIT(MUS_AUDIO_WRITE_ERROR, -1,
                                          mus_format("can't set default device to be %s", (chan == 4) ? "quad" : "stereo"));
                } else
                    err = MUS_ERROR;
            }
            break;
        case MUS_AUDIO_AMP:
            ...
            if(fld != -1)
            {
                ...
                if((fld == AL_LEFT_SPEAKER_GAIN) || (fld == AL_RIGHT_SPEAKER_GAIN))
                    ...
                    pb[1] = (1.0 - val[0]) * MAX_VOLUME;
                if(ALsetparams(AL_DEFAULT_DEVICE, pb, 2))
                    RETURN_ERROR_EXIT(MUS_AUDIO_WRITE_ERROR, -1, mus_format("can't set gain of %s", mus_audio_device_name(dev)));
            } else
                err = MUS_ERROR;
            break;
        case MUS_AUDIO_SRATE:
            ...
            if(fld != -1)
            {
                ...
                if(fld == AL_INPUT_RATE)
                {
                    ...
                    pb[1] = val[0];
                    if(ALsetparams(AL_DEFAULT_DEVICE, pb, 2))
                        RETURN_ERROR_EXIT(MUS_AUDIO_WRITE_ERROR, -1, mus_format("can't set srate of %s", mus_audio_device_name(dev)));
                }
            } else
                err = MUS_ERROR;
            break;
...
static void describe_audio_state_1(void)
{
    ...
    if(err == MUS_NO_ERROR)
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "speakers: %.2f", amps[0]);
        pprint(audio_strbuf);
    }
    ...
    if(err == MUS_NO_ERROR)
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "line in: %.2f", amps[0]);
        pprint(audio_strbuf);
    }
    ...
    if(err == MUS_NO_ERROR)
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "microphone: %.2f", amps[0]);
        pprint(audio_strbuf);
    }
    ...
    if(err == MUS_NO_ERROR)
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "line out: %.2f", amps[0]);
        pprint(audio_strbuf);
    }
    ...
    if(err == MUS_NO_ERROR)
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "digital out: %.2f", amps[0]);
        pprint(audio_strbuf);
    }
...
#define RETURN_ERROR_EXIT(Message_Type, Audio_Line, Ur_Message)                                                        \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            mus_print("%s\n  [%s[%d] %s]", Message, __FILE__, __LINE__, c__FUNCTION__);                                \
            FREE(Message);                                                                                             \
        } else                                                                                                         \
            mus_print("%s\n  [%s[%d] %s]", mus_error_type_to_string(Message_Type), __FILE__, __LINE__, c__FUNCTION__); \
        return (MUS_ERROR);                                                                                            \
    } while(false)
...
static void oss_mus_oss_set_buffers(int num, int size)
{
    FRAGMENTS = num;
    FRAGMENT_SIZE = size;
...
static char* oss_mus_audio_moniker(void)
{
    ...
    if(version_name == NULL)
        version_name = (char*)CALLOC(LABEL_BUFFER_SIZE, sizeof(char));
    if(SOUND_VERSION < 361)
    {
        ...
        mus_snprintf(version_name, LABEL_BUFFER_SIZE, "OSS %x.%x.%x", (SOUND_VERSION >> 16) & 0xff, (SOUND_VERSION >> 8) & 0xff,
                     SOUND_VERSION & 0xff);
    return (version_name);
}
#endif
...
static int oss_mus_audio_initialize(void)
{
    ...
    if(!audio_initialized)
    {
        ...
        for(i = 0; i < MAX_SOUNDCARDS; i++)
            mixer_state[i] = (int*)CALLOC(MIXER_SIZE, sizeof(int));
        for(i = 0; i < MAX_SOUNDCARDS; i++)
        {
            audio_fd[i] = -1;
            audio_open_ctr[i] = 0;
...
#if HAVE_SAM_9407
        {
            ...
            for(i = 0; i < MAX_SOUNDCARDS; i++)
            {
                ...
                if(fd < 0)
                    break;
                if((ioctl(fd, SAM_IOC_API_INFO, &apiInfo) < 0) || (apiInfo.apiClass != SAM_API_CLASS_VANILLA)
                   || (ioctl(fd, SAM_IOC_DRIVER_INFO, &driverInfo) < 0) || (!driverInfo.haveAudio))
                {
                    close(fd);
                    continue;
                }
        ...
        if(fd == -1)
            fd = open(SYNTH_NAME, O_RDONLY | O_NONBLOCK, 0);
        if(fd == -1)
        ...
        while((nmix < num_mixers) && (ndsp < num_dsps))
        {
            ...
            if(md == -1)
            {
                if(errno == EBUSY)
                {
                    mus_print("%s is busy: can't access it [%s[%d] %s]", dname, __FILE__, __LINE__, c__FUNCTION__);
                    nmix++;
                    continue;
                } else
                    break;
            ...
            if(fd == -1)
                fd = open(dname, O_RDONLY | O_NONBLOCK, 0);
            if(fd == -1)
                fd = open(dname, O_WRONLY | O_NONBLOCK, 0); /* some output devices need this */
            if(fd == -1)
            {
                close(md);
            ...
            if((status == 0) && (mixinfo.name) && (*(mixinfo.name)) && (strlen(mixinfo.name) > 6))
            {
...
static int linux_audio_open_with_error(const char* pathname, int flags, mode_t mode, int system)
{
    ...
    if(fd == -1)
        MUS_STANDARD_IO_ERROR(MUS_AUDIO_CANT_OPEN,
                              ((mode == O_RDONLY) ? "open read" : (mode == O_WRONLY) ? "open write" : "open read/write"), pathname);
    return (fd);
}
...
static int linux_audio_close(int fd)
{
    if(fd != -1)
    {
        ...
        if(sys != -1)
        {
            ...
            err = close(fd);
        if(err)
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_CLOSE, -1, mus_format("close %d failed: %s", fd, strerror(errno)));
    }
    /* is this an error? */
    return (MUS_NO_ERROR);
}
...
static int oss_mus_audio_open_output(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    if(oss_format == MUS_ERROR)
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, -1,
                          mus_format("format %d (%s) not available", format, mus_data_format_name(format)));
    if(audio_type[sys] == SONORUS_STUDIO)
    {
        ...
        switch(dev)
        {
            case MUS_AUDIO_DEFAULT:
                ...
                if(audio_out == -1)
                    audio_out = open("/dev/dsp", O_WRONLY, 0);
                break;
            case MUS_AUDIO_SPEAKERS:
                audio_out = open(sonorus_name(sys, 0), O_WRONLY, 0);
                if(audio_out == -1)
            ...
            case MUS_AUDIO_SPDIF_OUT: audio_out = open(sonorus_name(sys, 1), O_WRONLY, 0); break;
            case MUS_AUDIO_AES_OUT: audio_out = open(sonorus_name(sys, 9), O_WRONLY, 0); break;
            default:
                RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, audio_out,
                                  mus_format("Sonorus device %d (%s) not available", dev, mus_audio_device_name(dev)));
                break;
        }
        if(audio_out == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, audio_out,
                              mus_format("can't open Sonorus output device %d (%s): %s", dev, mus_audio_device_name(dev), strerror(errno)));
#ifdef NEW_OSS
        if(ioctl(audio_out, SNDCTL_DSP_CHANNELS, &chans) == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_out,
                              mus_format("can't get %d channels for Sonorus device %d (%s)", chans, dev, mus_audio_device_name(dev)));
#endif
        return (audio_out);
    }
    ...
    if(audio_type[sys] == SAM9407_DSP)
    {
        ...
        if(audio_out == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, audio_out, mus_format("can't open %s: %s", dname, strerror(errno)));
        if((ioctl(audio_out, SNDCTL_DSP_SETFMT, &oss_format) == -1) || (oss_format != to_oss_format(format)))
            RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, audio_out,
                              mus_format("can't set %s format to %d (%s)", dname, format, mus_data_format_name(format)));
        if(ioctl(audio_out, SNDCTL_DSP_CHANNELS, &chans) == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_out, mus_format("can't get %d channels on %s", chans, dname));
        if(ioctl(audio_out, SNDCTL_DSP_SPEED, &srate) == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_SRATE_NOT_AVAILABLE, audio_out, mus_format("can't set srate to %d on %s", srate, dname));
        FRAGMENT_SIZE = 14;
    ...
    if(dev == MUS_AUDIO_DEFAULT)
        ...
        audio_out = linux_audio_open_with_error(dev_name = dac_name(sys, (dev == MUS_AUDIO_AUX_OUTPUT) ? 1 : 0), O_RDWR, 0, sys);
    if(audio_out == -1)
        return (MUS_ERROR);

    /* ioctl(audio_out, SNDCTL_DSP_RESET, 0); */ /* causes clicks */
    if((fragments_locked) && (!(fragment_set_failed))
       && ((dev == MUS_AUDIO_DUPLEX_DEFAULT) || (size != 0))) /* only set if user has previously called set_oss_buffers */
    {
        buffer_info = (FRAGMENTS << 16) | (FRAGMENT_SIZE);
        if(ioctl(audio_out, SNDCTL_DSP_SETFRAGMENT, &buffer_info) == -1)
        {
            ...
            {
                char* tmp;
                fprintf(stderr, tmp = mus_format("can't set %s fragments to: %d x %d", dev_name, FRAGMENTS,
                                                 FRAGMENT_SIZE)); /* not an error if ALSA OSS-emulation */
    ...
    if((ioctl(audio_out, SNDCTL_DSP_SETFMT, &oss_format) == -1) || (oss_format != to_oss_format(format)))
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, audio_out,
                          mus_format("data format %d (%s) not available on %s", format, mus_data_format_name(format), dev_name));
#ifdef NEW_OSS
    if(ioctl(audio_out, SNDCTL_DSP_CHANNELS, &chans) == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_out, mus_format("can't get %d channels on %s", chans, dev_name));
#else
    if(chans == 2)
        stereo = 1;
    else
        stereo = 0;
    if((ioctl(audio_out, SNDCTL_DSP_STEREO, &stereo) == -1) || ((chans == 2) && (stereo == 0)))
        RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_out, mus_format("can't get %d channels on %s", chans, dev_name));
#endif
    if(ioctl(audio_out, SNDCTL_DSP_SPEED, &srate) == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_SRATE_NOT_AVAILABLE, audio_out, mus_format("can't set srate of %s to %d", dev_name, srate));
...
static int oss_mus_audio_write(int line, char* buf, int bytes)
{
    ...
    if(err != bytes)
    {
        if(errno != 0)
            ...
            RETURN_ERROR_EXIT(MUS_AUDIO_WRITE_ERROR, -1, mus_format("wrote %d bytes of requested %d", err, bytes));
    }
    return (MUS_NO_ERROR);
}
...
static int oss_mus_audio_read(int line, char* buf, int bytes)
{
    ...
    {
        if(errno != 0)
            RETURN_ERROR_EXIT(MUS_AUDIO_READ_ERROR, -1, mus_format("read error: %s", strerror(errno)));
        else
            RETURN_ERROR_EXIT(MUS_AUDIO_READ_ERROR, -1, mus_format("read %d bytes of requested %d", err, bytes));
    }
    return (MUS_NO_ERROR);
}
...
static char* oss_unsrc(int srcbit)
{
    ...
    else
    {
        ...
        if(srcbit & SOUND_MASK_MIC)
        {
            need_and = true;
            strcat(buf, "mic");
        }
        if(srcbit & SOUND_MASK_LINE)
        {
            if(need_and)
                strcat(buf, " and ");
            need_and = true;
...
static int oss_mus_audio_open_input(int ur_dev, int srate, int chans, int format, int requested_size)
{
    ...
    if(audio_type[sys] == SONORUS_STUDIO)
    {
        ...
        switch(dev)
        {
            case MUS_AUDIO_DEFAULT:
                ...
                break;
            case MUS_AUDIO_ADAT_IN: audio_fd = open(dev_name = sonorus_name(sys, 11), O_RDONLY, 0); break;
            case MUS_AUDIO_AES_IN: audio_fd = open(dev_name = sonorus_name(sys, 20), O_RDONLY, 0); break;
            case MUS_AUDIO_SPDIF_IN: audio_fd = open(dev_name = sonorus_name(sys, 5), O_RDONLY, 0); break;
            default:
                RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, -1, mus_format("no %s device on Sonorus?", mus_audio_device_name(dev)));
                break;
        }
        if(audio_fd == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_NO_INPUT_AVAILABLE, -1,
                              mus_format("can't open %s (Sonorus device %s): %s", dev_name, mus_audio_device_name(dev), strerror(errno)));
#ifdef NEW_OSS
        if(ioctl(audio_fd, SNDCTL_DSP_CHANNELS, &chans) == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_fd,
                              mus_format("can't get %d channels on %s (Sonorus device %s)", chans, dev_name, mus_audio_device_name(dev)));
#endif
        return (audio_fd);
    }
    ...
    if(audio_type[sys] == SAM9407_DSP)
    {
        ...
        if(audio_fd == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, audio_fd, mus_format("can't open input %s: %s", dname, strerror(errno)));
        if((ioctl(audio_fd, SNDCTL_DSP_SETFMT, &oss_format) == -1) || (oss_format != to_oss_format(format)))
            RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, audio_fd,
                              mus_format("can't set %s format to %d (%s)", dname, format, mus_data_format_name(format)));
        if(ioctl(audio_fd, SNDCTL_DSP_CHANNELS, &chans) == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_fd, mus_format("can't get %d channels on %s", chans, dname));
        if(ioctl(audio_fd, SNDCTL_DSP_SPEED, &srate) == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_SRATE_NOT_AVAILABLE, audio_fd, mus_format("can't set srate to %d on %s", srate, dname));
        FRAGMENT_SIZE = 14;
        buffer_info = (FRAGMENTS << 16) | (FRAGMENT_SIZE);
    ...
    if(((dev == MUS_AUDIO_DEFAULT) || (dev == MUS_AUDIO_DUPLEX_DEFAULT)) && (sys == 0))
        ...
        audio_fd = linux_audio_open(dev_name = dac_name(sys, (dev == MUS_AUDIO_AUX_INPUT) ? 1 : 0), O_RDONLY, 0, sys);
    if(audio_fd == -1)
    {
        if(dev == MUS_AUDIO_DUPLEX_DEFAULT)
            RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, -1,
                              mus_format("can't open %s (device %s): %s", dev_name, mus_audio_device_name(dev), strerror(errno)));
    ...
    switch(dev)
    {
        ...
        case MUS_AUDIO_LINE3:
            srcbit = SOUND_MASK_LINE3;
            break; /* also digital1..3 */
        case MUS_AUDIO_DUPLEX_DEFAULT:
    ...
    if((ioctl(audio_fd, SNDCTL_DSP_SETFMT, &oss_format) == -1) || (oss_format != to_oss_format(format)))
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, audio_fd,
                          mus_format("can't set %s format to %d (%s)", dev_name, format, mus_data_format_name(format)));
#ifdef NEW_OSS
    if(ioctl(audio_fd, SNDCTL_DSP_CHANNELS, &chans) == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_fd, mus_format("can't get %d channels on %s", chans, dev_name));
#else
    if(chans == 2)
        stereo = 1;
    else
        stereo = 0;
    if((ioctl(audio_fd, SNDCTL_DSP_STEREO, &stereo) == -1) || ((chans == 2) && (stereo == 0)))
        RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_fd,
                          mus_format("can't get %d channels on %s (%s)", chans, dev_name, mus_audio_device_name(dev)));
#endif
    if(ioctl(audio_fd, SNDCTL_DSP_SPEED, &srate) == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_SRATE_NOT_AVAILABLE, audio_fd,
                          mus_format("can't set srate to %d on %s (%s)", srate, dev_name, mus_audio_device_name(dev)));
    return (audio_fd);
}
...
static int oss_mus_audio_mixer_read(int ur_dev, int field, int chan, float* val)
{
                ...
                } else
                {
                    if(field == MUS_AUDIO_CHANNEL)
                    {
                        switch(dev)
                        {
                            ...
                            case MUS_AUDIO_ADAT_OUT: channels = 8; break;
                            case MUS_AUDIO_AES_IN:
                            case MUS_AUDIO_AES_OUT: channels = 2; break;
                            case MUS_AUDIO_SPDIF_IN:
                            case MUS_AUDIO_SPDIF_OUT: channels = 4; break;
                            case MUS_AUDIO_DEFAULT:
                                if(adat_mode)
                                    channels = 8;
                                else
    ...
    if(audio_type[sys] == SAM9407_DSP)
    {
        switch(field)
        {
            ...
            case MUS_AUDIO_AMP:
                RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, -1,
                                  mus_format("can't read %s's gains in Sam9407", mus_audio_device_name(dev)));
                break;
            case MUS_AUDIO_SRATE: val[0] = 44100; break;
            default:
                RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, -1,
                                  mus_format("can't read %s's %s in Sam9407", mus_audio_device_name(dev), mus_audio_device_name(field)));
                break;
        }
        return (MUS_NO_ERROR);
    }
                ...
                } else
                {
                    if(field == MUS_AUDIO_CHANNEL)
                    {
                        switch(dev)
                        {
                            ...
                            case MUS_AUDIO_SPDIF_IN:
                            case MUS_AUDIO_SPDIF_OUT: channels = 4; break;
                            case MUS_AUDIO_DEFAULT: channels = 8; break;
                            default: channels = 0; break;
                        }
                        val[0] = channels;
                    } else
        ...
        if(fd == -1)
        {
            ...
            if(fd == -1)
            {
                RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, -1,
                                  mus_format("can't open input %s or %s: %s", dev_name, DAC_NAME, strerror(errno)));
                return (MUS_ERROR);
            } else
                dev_name = DAC_NAME;
        } else
    ...
    if(ioctl(fd, SOUND_MIXER_READ_DEVMASK, &devmask))
        RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, fd, mus_format("can't read device info from %s", dev_name));
    err = 0;
    if((dev == MUS_AUDIO_MIXER) || (dev == MUS_AUDIO_DAC_FILTER)) /* these give access to all the on-board analog input gain controls */
    {
        ...
        switch(field)
        {
            ...
            case MUS_AUDIO_IMIX:
                if(SOUND_MASK_IMIX & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_IMIX), &amp);
                break;
            case MUS_AUDIO_IGAIN:
                if(SOUND_MASK_IGAIN & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_IGAIN), &amp);
                break;
            case MUS_AUDIO_RECLEV:
                if(SOUND_MASK_RECLEV & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_RECLEV), &amp);
                break;
            case MUS_AUDIO_PCM:
                if(SOUND_MASK_PCM & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_PCM), &amp);
                break;
            case MUS_AUDIO_PCM2:
                if(SOUND_MASK_ALTPCM & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_ALTPCM), &amp);
                break;
            case MUS_AUDIO_OGAIN:
                if(SOUND_MASK_OGAIN & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_OGAIN), &amp);
                break;
            case MUS_AUDIO_LINE:
                if(SOUND_MASK_LINE & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_LINE), &amp);
                break;
            case MUS_AUDIO_MICROPHONE:
                if(SOUND_MASK_MIC & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_MIC), &amp);
                break;
            case MUS_AUDIO_LINE1:
                if(SOUND_MASK_LINE1 & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_LINE1), &amp);
                break;
            case MUS_AUDIO_LINE2:
                if(SOUND_MASK_LINE2 & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_LINE2), &amp);
                break;
            case MUS_AUDIO_LINE3:
                if(SOUND_MASK_LINE3 & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_LINE3), &amp);
                break;
            case MUS_AUDIO_SYNTH:
                if(SOUND_MASK_SYNTH & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_SYNTH), &amp);
                break;
            case MUS_AUDIO_BASS:
                if(SOUND_MASK_BASS & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_BASS), &amp);
                break;
            case MUS_AUDIO_TREBLE:
                if(SOUND_MASK_TREBLE & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_TREBLE), &amp);
                break;
            case MUS_AUDIO_CD:
                if(SOUND_MASK_CD & devmask)
                    err = ioctl(fd, MIXER_READ(SOUND_MIXER_CD), &amp);
                break;
            case MUS_AUDIO_CHANNEL:
                if(dev == MUS_AUDIO_MIXER)
                {
                    ...
                    if(SOUND_MASK_IGAIN & devmask)
                    {
                        if(SOUND_MASK_IGAIN & stereodevs)
                            channels += 2;
                        else
                            channels += 1;
                    }
                    if(SOUND_MASK_RECLEV & devmask)
                    {
                        if(SOUND_MASK_RECLEV & stereodevs)
                            channels += 2;
                        else
                            channels += 1;
                    }
                    ...
                    if(SOUND_MASK_ALTPCM & devmask)
                    {
                        if(SOUND_MASK_ALTPCM & stereodevs)
                            channels += 2;
                        else
                            channels += 1;
                    }
                    if(SOUND_MASK_OGAIN & devmask)
                    {
                        if(SOUND_MASK_OGAIN & stereodevs)
                            channels += 2;
                        else
                            channels += 1;
                    }
                    ...
                    if(SOUND_MASK_LINE1 & devmask)
                    {
                        if(SOUND_MASK_LINE1 & stereodevs)
                            channels += 2;
                        else
                            channels += 1;
                    }
                    if(SOUND_MASK_LINE2 & devmask)
                    {
                        if(SOUND_MASK_LINE2 & stereodevs)
                            channels += 2;
                        else
                            channels += 1;
                    }
                    if(SOUND_MASK_LINE3 & devmask)
                    {
                        if(SOUND_MASK_LINE3 & stereodevs)
                            channels += 2;
                        else
                            channels += 1;
                    }
                    if(SOUND_MASK_SYNTH & devmask)
                    {
                        if(SOUND_MASK_SYNTH & stereodevs)
                            channels += 2;
                        else
                            channels += 1;
                    }
                    if(SOUND_MASK_CD & devmask)
                    {
                        if(SOUND_MASK_CD & stereodevs)
                            channels += 2;
                        else
                            channels += 1;
                    }
                } else if(SOUND_MASK_TREBLE & devmask)
                    channels = 2;
                else
                    channels = 0;
                val[0] = channels;
            ...
            case MUS_AUDIO_FORMAT: /* this is asking for configuration info -- we return an array with per-"device" channels */
                ioctl(fd, SOUND_MIXER_READ_STEREODEVS, &stereodevs);
                for(ind = 0; ind <= MUS_AUDIO_SYNTH; ind++)
                {
                    if(chan > ind)
                        val[ind] = 0;
                }
                if(SOUND_MASK_IMIX & devmask)
                {
                    if(chan > MUS_AUDIO_IMIX)
                        val[MUS_AUDIO_IMIX] = ((SOUND_MASK_IMIX & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_IGAIN & devmask)
                {
                    if(chan > MUS_AUDIO_IGAIN)
                        val[MUS_AUDIO_IGAIN] = ((SOUND_MASK_IGAIN & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_RECLEV & devmask)
                {
                    if(chan > MUS_AUDIO_RECLEV)
                        val[MUS_AUDIO_RECLEV] = ((SOUND_MASK_RECLEV & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_PCM & devmask)
                {
                    if(chan > MUS_AUDIO_PCM)
                        val[MUS_AUDIO_PCM] = ((SOUND_MASK_PCM & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_ALTPCM & devmask)
                {
                    if(chan > MUS_AUDIO_PCM2)
                        val[MUS_AUDIO_PCM2] = ((SOUND_MASK_ALTPCM & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_OGAIN & devmask)
                {
                    if(chan > MUS_AUDIO_OGAIN)
                        val[MUS_AUDIO_OGAIN] = ((SOUND_MASK_OGAIN & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_LINE & devmask)
                {
                    if(chan > MUS_AUDIO_LINE)
                        val[MUS_AUDIO_LINE] = ((SOUND_MASK_LINE & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_MIC & devmask)
                {
                    if(chan > MUS_AUDIO_MICROPHONE)
                        val[MUS_AUDIO_MICROPHONE] = ((SOUND_MASK_MIC & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_LINE1 & devmask)
                {
                    if(chan > MUS_AUDIO_LINE1)
                        val[MUS_AUDIO_LINE1] = ((SOUND_MASK_LINE1 & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_LINE2 & devmask)
                {
                    if(chan > MUS_AUDIO_LINE2)
                        val[MUS_AUDIO_LINE2] = ((SOUND_MASK_LINE2 & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_LINE3 & devmask)
                {
                    if(chan > MUS_AUDIO_LINE3)
                        val[MUS_AUDIO_LINE3] = ((SOUND_MASK_LINE3 & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_SYNTH & devmask)
                {
                    if(chan > MUS_AUDIO_SYNTH)
                        val[MUS_AUDIO_SYNTH] = ((SOUND_MASK_SYNTH & stereodevs) ? 2 : 1);
                }
                if(SOUND_MASK_CD & devmask)
                {
                    if(chan > MUS_AUDIO_CD)
                        val[MUS_AUDIO_CD] = ((SOUND_MASK_CD & stereodevs) ? 2 : 1);
                }
                linux_audio_close(fd);
                return (MUS_NO_ERROR);
                break;
            default:
                RETURN_ERROR_EXIT(
                  MUS_AUDIO_CANT_READ, fd,
                  mus_format("can't read %s's (%s) %s", mus_audio_device_name(dev), dev_name, mus_audio_device_name(field)));
                break;
        }
        if(chan == 0)
            val[0] = ((float)(amp & 0xff)) * 0.01;
        else
            val[0] = (((float)((amp & 0xff00) >> 8)) * 0.01);
    } else
    {
        switch(field)
        {
            case MUS_AUDIO_PORT:
                ...
                if((SOUND_MASK_MIC | SOUND_MASK_LINE | SOUND_MASK_CD) & devmask)
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_AUDIO_LINE_IN;
                }
                /* problem here is that microphone and line_in are mixed before the ADC */
                if(SOUND_MASK_SPEAKER & devmask)
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_AUDIO_SPEAKERS;
                }
                if(SOUND_MASK_VOLUME & devmask)
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_AUDIO_DAC_OUT;
                }
                if(SOUND_MASK_TREBLE & devmask)
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_AUDIO_DAC_FILTER;
                }
            ...
            case MUS_AUDIO_FORMAT:
                ...
                if(fd == -1)
                {
                    RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, -1, mus_format("can't open %s: %s", DAC_NAME, strerror(errno)));
                    return (MUS_ERROR);
                }
...
#endif
                ind = 0;
                if(formats & (to_oss_format(MUS_BYTE)))
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_BYTE;
                }
                if(formats & (to_oss_format(MUS_BSHORT)))
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_BSHORT;
                }
                if(formats & (to_oss_format(MUS_UBYTE)))
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_UBYTE;
                }
                if(formats & (to_oss_format(MUS_MULAW)))
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_MULAW;
                }
                if(formats & (to_oss_format(MUS_ALAW)))
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_ALAW;
                }
                if(formats & (to_oss_format(MUS_LSHORT)))
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_LSHORT;
                }
                if(formats & (to_oss_format(MUS_UBSHORT)))
                {
                    ind++;
                    if(chan > ind)
                        val[ind] = MUS_UBSHORT;
            ...
            case MUS_AUDIO_CHANNEL:
                ...
                switch(dev)
                {
                    ...
                    case MUS_AUDIO_SPEAKERS:
                        if(SOUND_MASK_SPEAKER & devmask)
                        {
                            if(SOUND_MASK_SPEAKER & stereodevs)
                                channels = 2;
                            else
                                channels = 1;
                        }
                    ...
                    case MUS_AUDIO_DAC_OUT:
                        if(SOUND_MASK_VOLUME & devmask)
                        {
                            if(SOUND_MASK_VOLUME & stereodevs)
                                channels = 2;
                            else
                                channels = 1;
                        }
                    ...
                    default:
                        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, fd,
                                          mus_format("can't read channel info from %s (%s)", mus_audio_device_name(dev), dev_name));
                        break;
                }
                val[0] = channels;
                break;
            case MUS_AUDIO_AMP:
                ...
                switch(dev)
                {
                    ...
                    case MUS_AUDIO_SPEAKERS:
                        if(SOUND_MASK_SPEAKER & devmask)
                            err = ioctl(fd, MIXER_READ(SOUND_MIXER_SPEAKER), &amp);
                        break;
                    case MUS_AUDIO_LINE_IN:
                        if(SOUND_MASK_LINE & devmask)
                            err = ioctl(fd, MIXER_READ(SOUND_MIXER_LINE), &amp);
                    ...
                    case MUS_AUDIO_DAC_OUT:
                        if(SOUND_MASK_VOLUME & devmask)
                            err = ioctl(fd, MIXER_READ(SOUND_MIXER_VOLUME), &amp);
                        break;
                    case MUS_AUDIO_DEFAULT:
                        if(SOUND_MASK_VOLUME & devmask)
                            err = ioctl(fd, MIXER_READ(SOUND_MIXER_VOLUME), &amp);
                    ...
                    default:
                        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, fd,
                                          mus_format("can't get gain info for %s (%s)", mus_audio_device_name(dev), dev_name));
                        break;
                }
                if(err)
                    RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, fd,
                                      mus_format("can't read %s's (%s) amp info", mus_audio_device_name(dev), dev_name));
                if(chan == 0)
                    val[0] = ((float)(amp & 0xff)) * 0.01;
                else
                    val[0] = (((float)((amp & 0xff00) >> 8)) * 0.01);
                ...
                if(ioctl(fd, SNDCTL_DSP_SPEED, &srate) == -1)
                {
                    ...
                    if(ioctl(fd, SNDCTL_DSP_SPEED, &srate) == -1)
                        RETURN_ERROR_EXIT(MUS_AUDIO_SRATE_NOT_AVAILABLE, fd,
                                          mus_format("can't get %s's (%s) srate", mus_audio_device_name(dev), dev_name));
                }
                val[0] = (float)srate;
                break;
            case MUS_AUDIO_DIRECTION:
                switch(dev)
                {
                    ...
                    case MUS_AUDIO_DAC_OUT: val[0] = 0.0; break;
                    default: val[0] = 1.0; break;
                }
            default:
                RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, fd,
                                  mus_format("can't get %s's (%s) %s", mus_audio_device_name(dev), dev_name, mus_audio_device_name(field)));
                break;
        }
    }
...
static int oss_mus_audio_mixer_write(int ur_dev, int field, int chan, float* val)
{
...
#if HAVE_SAM_9407
    if(audio_type[sys] == SAM9407_DSP)
        return (MUS_NO_ERROR); /* XXX */
#endif

    if(audio_type[sys] == SONORUS_STUDIO)
        return (MUS_NO_ERROR); /* there are apparently volume controls, but they're not accessible yet */
    if(audio_type[sys] == RME_HAMMERFALL)
        return (MUS_NO_ERROR);
    if(audio_type[sys] == DELTA_66)
        return (MUS_NO_ERROR);
    ...
    if((dev == MUS_AUDIO_MIXER) || (dev == MUS_AUDIO_DAC_FILTER)) /* these give access to all the on-board analog input gain controls */
    {
        ...
        if(chan == 0)
            ...
            vol = (((int)(val[0] * 100)) << 8) + ((int)(amp[0] * 100));
        ioctl(fd, SOUND_MIXER_READ_DEVMASK, &devmask);
        switch(field)
        {
            case MUS_AUDIO_IMIX:
                if(SOUND_MASK_IMIX & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_IMIX), &vol);
                break;
            case MUS_AUDIO_IGAIN:
                if(SOUND_MASK_IGAIN & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_IGAIN), &vol);
                break;
            case MUS_AUDIO_RECLEV:
                if(SOUND_MASK_RECLEV & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_RECLEV), &vol);
                break;
            case MUS_AUDIO_PCM:
                if(SOUND_MASK_PCM & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_PCM), &vol);
                break;
            case MUS_AUDIO_PCM2:
                if(SOUND_MASK_ALTPCM & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_ALTPCM), &vol);
                break;
            case MUS_AUDIO_OGAIN:
                if(SOUND_MASK_OGAIN & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_OGAIN), &vol);
                break;
            case MUS_AUDIO_LINE:
                if(SOUND_MASK_LINE & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_LINE), &vol);
                break;
            case MUS_AUDIO_MICROPHONE:
                if(SOUND_MASK_MIC & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_MIC), &vol);
                break;
            case MUS_AUDIO_LINE1:
                if(SOUND_MASK_LINE1 & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_LINE1), &vol);
                break;
            case MUS_AUDIO_LINE2:
                if(SOUND_MASK_LINE2 & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_LINE2), &vol);
                break;
            case MUS_AUDIO_LINE3:
                if(SOUND_MASK_LINE3 & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_LINE3), &vol);
                break;
            case MUS_AUDIO_SYNTH:
                if(SOUND_MASK_SYNTH & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_SYNTH), &vol);
                break;
            case MUS_AUDIO_BASS:
                if(SOUND_MASK_BASS & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_BASS), &vol);
                break;
            case MUS_AUDIO_TREBLE:
                if(SOUND_MASK_TREBLE & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_TREBLE), &vol);
                break;
            case MUS_AUDIO_CD:
                if(SOUND_MASK_CD & devmask)
                    err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_CD), &vol);
                break;
            default:
                RETURN_ERROR_EXIT(
                  MUS_AUDIO_CANT_WRITE, fd,
                  mus_format("can't write %s's (%s) %s field", mus_audio_device_name(dev), dev_name, mus_audio_device_name(field)));
                break;
        }
    } else
    {
        switch(field)
        {
            case MUS_AUDIO_AMP:
                ...
                switch(dev)
                {
                    ...
                    case MUS_AUDIO_SPEAKERS:
                        if(SOUND_MASK_SPEAKER & devmask)
                            err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_SPEAKER), &vol);
                        break;
                    case MUS_AUDIO_LINE_IN:
                        if(SOUND_MASK_LINE & devmask)
                            err = ioctl(fd, MIXER_WRITE(SOUND_MIXER_LINE), &vol);
                    ...
                    default:
                        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, fd,
                                          mus_format("device %d (%s) not available on %s", dev, mus_audio_device_name(dev), dev_name));
                }
                break;
            case MUS_AUDIO_SRATE:
                vol = (int)val[0];
    ...
    if(err)
        RETURN_ERROR_EXIT(
          MUS_AUDIO_WRITE_ERROR, fd,
          mus_format("possible write problem for %s's (%s) %s field", mus_audio_device_name(dev), dev_name, mus_audio_device_name(field)));
    return (linux_audio_close(fd));
}
...
static char* synth_names[] = {"",
                              ...
                              "MPU 401",
                              "SoundBlaster 16",
                              "SoundBlaster 16 MIDI",
                              "6850 UART",
                              "Gravis UltraSound 16",
                              "Microsoft",
                              "Personal sound system",
                              "Ensoniq Soundscape",
                              "Personal sound system + MPU",
                              "Personal/Microsoft",
                              "Mediatrix Pro",
                              "MAD16",
                              "MAD16 + MPU",
...
static void oss_describe_audio_state_1(void)
{
    ...
    mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "%d card%s found", sound_cards, (sound_cards != 1) ? "s" : "");
    pprint(audio_strbuf);
    if(sound_cards > 1)
    {
        pprint(": ");
        for(i = 0; i < sound_cards; i++)
        {
            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "/dev/dsp%d with /dev/mixer%d%s", audio_dsp[i], audio_mixer[i],
                         (i < (sound_cards - 1)) ? ", " : "");
            pprint(audio_strbuf);
        }
    }
            ...
            for(i = 0; i < numdevs; i++)
            {
                ...
                if(status != -1)
                {
                    mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "  device: %d: %s, %s, %d voices\n", i, sinfo.name,
                                 device_type(sinfo.synth_type), sinfo.nr_voices);
                    pprint(audio_strbuf);
                }
            }
    ...
    if(fd == -1)
    {
        ...
        if(fd == -1)
        {
            if(dsp_num == 0)
            {
                ...
                goto AUDIO_INFO; /* no /dev/mixern */
        } else
            pprint("no audio input enabled\n");
    }
    if(fd == -1)
        goto AUDIO_INFO;
        ...
        } else
        {
            ...
            for(i = 0; i < SOUND_MIXER_NRDEVICES; i++)
            {
                if((1 << i) & devmask)
                {
                    ...
                    {
                        if((1 << i) & stereodevs)
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "  %.2f %.2f", (float)(level & 0xff) * 0.01,
                                         (float)((level & 0xff00) >> 8) * 0.01);
                        else
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "  %.2f", (float)(level & 0xff) * 0.01);
...
AUDIO_INFO:
    ...
    pprint(audio_strbuf);
    if((ioctl(fd, SOUND_PCM_READ_RATE, &rate) != -1) && (ioctl(fd, SOUND_PCM_READ_CHANNELS, &channels) != -1)
       && (ioctl(fd, SOUND_PCM_READ_BITS, &bits) != -1) && (ioctl(fd, SNDCTL_DSP_GETBLKSIZE, &blocksize) != -1))
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE,
                     "  defaults:\n    sampling rate: %d, chans: %d, sample size: %d bits, block size: %d bytes", rate, channels, bits,
                     blocksize);
        pprint(audio_strbuf);
        ...
        deffmt = AFMT_QUERY;
        if((ioctl(fd, SOUND_PCM_SETFMT, &deffmt) != -1) && (ioctl(fd, SOUND_PCM_GETFMTS, &formats) != -1))
        {
            pprint("  supported formats:\n");
            if(formats & AFMT_MU_LAW)
            {
                pprint("    mulaw");
                if(deffmt == AFMT_MU_LAW)
                    pprint(" (default)");
                pprint("\n");
            }
            ...
            {
                pprint("    adpcm");
                if(deffmt == AFMT_IMA_ADPCM)
                    pprint(" (default)");
                pprint("\n");
            }
            if(formats & AFMT_U8)
            {
                pprint("    unsigned byte");
                if(deffmt == AFMT_U8)
                    pprint(" (default)");
                pprint("\n");
            }
            ...
            {
                pprint("    signed little-endian short");
                if(deffmt == AFMT_S16_LE)
                    pprint(" (default)");
                pprint("\n");
            }
            if(formats & AFMT_S16_BE)
            {
                pprint("    signed big-endian short");
                if(deffmt == AFMT_S16_BE)
                    pprint(" (default)");
                pprint("\n");
            }
            if(formats & AFMT_S8)
            ...
            {
                pprint("    unsigned little-endian short");
                if(deffmt == AFMT_U16_LE)
                    pprint(" (default)");
                pprint("\n");
            }
            if(formats & AFMT_U16_BE)
            {
                pprint("    unsigned big-endian short");
                if(deffmt == AFMT_U16_BE)
                    pprint(" (default)");
                pprint("\n");
            }
            ...
            {
                pprint("    signed little-endian int");
                if(deffmt == AFMT_S32_LE)
                    pprint(" (default)");
                pprint("\n");
            }
            if(formats & AFMT_S32_BE)
            {
                pprint("    signed big-endian int");
                if(deffmt == AFMT_S32_BE)
                    pprint(" (default)");
                pprint("\n");
            }
#endif
            ...
            if(status != -1)
            {
                ...
                for(channels = 1; channels <= 2; channels++)
                {
                    ...
                    {
                        min_rate = 1;
                        if(set_dsp(fd, channels, bits, &min_rate) == -1)
                            continue;
                        max_rate = 100000;
                        if(set_dsp(fd, channels, bits, &max_rate) == -1)
                            continue;
...
static int to_mus_format(int alsa_format)
{
    ...
    switch(alsa_format)
    {
        ...
        case SND_PCM_FORMAT_S32_LE:
            return (MUS_LINTN); /* 32bit normalized plays 24bit and 16bit files with same amplitude bound (for 24 bit cards) */
...
static snd_pcm_hw_params_t* alsa_get_hardware_params(const char* name, snd_pcm_stream_t stream, int mode)
{
    ...
    {
        alsa_mus_error(MUS_AUDIO_CANT_OPEN, mus_format("open pcm %s for stream %d: %s", name, stream, snd_strerror(err)));
        return (NULL);
    } else
    {
        ...
        if(params == NULL)
        {
            ...
            alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, mus_format("could not allocate memory for hardware params"));
        } else
        {
            ...
            if(err < 0)
            {
                ...
                alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE,
                               mus_format("snd_pcm_hw_params_any: pcm %s, stream %d, error: %s", name, stream, snd_strerror(err)));
            } else
            {
                snd_pcm_close(handle);
                return (params);
...
static snd_pcm_sw_params_t* alsa_get_software_params(void)
{
    ...
    if(params == NULL)
    {
        alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, mus_format("could not allocate memory for software params"));
    }
    return (params);
}
...
static bool alsa_probe_device_name(const char* name)
{
    ...
    snd_config_for_each(pos, next, conf)
    {
        ...
        if(err == 0)
        {
            ...
            if(result == 0 && (name[strlen(id)] == '\0' || name[strlen(id)] == ':'))
            {
                return (true);
            }
        }
...
static int alsa_check_device_name(const char* name)
{
    ...
    {
        return (alsa_mus_error(MUS_AUDIO_CANT_READ, mus_format("alsa could not find device \"%s\" in configuration", name)));
    }
    return (MUS_NO_ERROR);
}
...
static bool alsa_set_playback_parameters(void)
{
    ...
    if(alsa_hw_params[SND_PCM_STREAM_PLAYBACK])
    {
        ...
        if(alsa_sw_params[SND_PCM_STREAM_PLAYBACK])
            free(alsa_sw_params[SND_PCM_STREAM_PLAYBACK]);
...
static bool alsa_set_capture_parameters(void)
{
    ...
    if(alsa_hw_params[SND_PCM_STREAM_CAPTURE])
    {
        ...
        int old_buffers;
        old_buffers = alsa_buffers;
        if(alsa_sw_params[SND_PCM_STREAM_CAPTURE])
            free(alsa_sw_params[SND_PCM_STREAM_CAPTURE]);
...
static int alsa_get_int_from_env(const char* name, int* value, int min, int max)
{
    ...
    if(string)
    {
        ...
        if(((min != -1) && (max != -1)) && (result < min || result > max))
        {
            return (alsa_mus_error(MUS_AUDIO_CANT_READ,
                                   mus_format("%s ignored: out of range, value=%d, min=%d, max=%d", name, (int)result, min, max)));
        } else
        {
            if(errno == ERANGE)
            {
                return (alsa_mus_error(MUS_AUDIO_CANT_READ, mus_format("%s ignored: strlol conversion out of range", name)));
            } else
            {
                if((*string != '\0') && (*end == '\0'))
                {
                ...
                {
                    return (alsa_mus_error(MUS_AUDIO_CANT_READ, mus_format("%s ignored: value is \"%s\", not an integer", name, string)));
                }
            }
        }
    }
...
static int alsa_mus_audio_initialize(void)
{
    ...
    if((!alsa_hw_params[SND_PCM_STREAM_CAPTURE]) || (!alsa_hw_params[SND_PCM_STREAM_PLAYBACK]))
        return (MUS_ERROR);
...
static int alsa_audio_open(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    if(alsa_trace)
        mus_print("%s: %x rate=%d, chans=%d, format=%d:%s, size=%d", c__FUNCTION__, ur_dev, srate, chans, format,
    ...
    {
        return (alsa_mus_error(MUS_AUDIO_DEVICE_NOT_AVAILABLE, mus_format("%s: cannot translate device %s<%d> to alsa", snd_strerror(err),
    ...
    if((alsa_format = to_alsa_format(format)) == (snd_pcm_format_t)MUS_ERROR)
    {
        return (alsa_mus_error(MUS_AUDIO_FORMAT_NOT_AVAILABLE,
                               mus_format("could not change %s<%d> to alsa format", mus_audio_format_name(format), format)));
    }
    ...
    {
        snd_pcm_close(handle);
        return (alsa_mus_error(MUS_AUDIO_CANT_OPEN, mus_format("open pcm %s (%s) stream %s: %s", mus_audio_device_name(device), alsa_name,
                                                               snd_pcm_stream_name(alsa_stream), snd_strerror(err))));
    }
    ...
    if((err = snd_pcm_hw_params_any(handle, hw_params)) < 0)
    {
        ...
        return (alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE,
                               mus_format("%s: no parameter configurations available for %s", snd_strerror(err), alsa_name)));
    }
    ...
    if(err < 0)
    {
        ...
        handles[alsa_stream] = NULL;
        alsa_dump_configuration(alsa_name, hw_params, sw_params);
        return (alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, mus_format("%s: %s: access type %s not available", snd_strerror(err),
                                                                                 alsa_name, snd_pcm_access_name(alsa_interleave))));
    }
    ...
    if(err < 0)
    {
        ...
        alsa_dump_configuration(alsa_name, hw_params, sw_params);
        return (alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE,
                               mus_format("%s: %s: cannot set number of periods to %d, min is %d, max is %d", snd_strerror(err), alsa_name,
                                          periods, (int)minp, (int)maxp)));
    }
...
total requested buffer size is %d frames, minimum allowed is %d, maximum is %d",
    ...
    if(err < 0)
    {
        ...
        handles[alsa_stream] = NULL;
        alsa_dump_configuration(alsa_name, hw_params, sw_params);
        return (alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE, mus_format("%s: %s: cannot set format to %s", snd_strerror(err),
                                                                                 alsa_name, snd_pcm_format_name(alsa_format))));
    }
    ...
    if(err < 0)
    {
        ...
        alsa_dump_configuration(alsa_name, hw_params, sw_params);
        return (alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE,
                               mus_format("%s: %s: cannot set channels to %d", snd_strerror(err), alsa_name, chans)));
    }
#if HAVE_NEW_ALSA
    {
        ...
        if(r < 0)
        {
            ...
            alsa_dump_configuration(alsa_name, hw_params, sw_params);
            return (alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE,
                                   mus_format("%s: %s: cannot set sampling rate near %d", snd_strerror(r), alsa_name, srate)));
        } else
        {
            if((new_rate != srate) && (!alsa_squelch_warning))
            {
                mus_print("%s: could not set rate to exactly %d, set to %d instead", alsa_name, srate, new_rate);
            }
        }
    }
#else
    ...
    } else
    {
        if(r != srate)
        {
            mus_print("%s: could not set rate to exactly %d, set to %d instead", alsa_name, srate, r);
        }
    }
#endif
    ...
    if(err < 0)
    {
        ...
        alsa_dump_configuration(alsa_name, hw_params, sw_params);
        return (alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE,
                               mus_format("%s: cannot set hardware parameters for %s", snd_strerror(err), alsa_name)));
    }
    ...
    if(err < 0)
    {
        ...
        alsa_dump_configuration(alsa_name, hw_params, sw_params);
        return (alsa_mus_error(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE,
                               mus_format("%s: cannot set software parameters for %s", snd_strerror(err), alsa_name)));
    }
...
static int alsa_mus_audio_close(int id)
{
    ...
    xrun_warned = false;
    if(alsa_trace)
        mus_print("%s: %d", c__FUNCTION__, id);
    if(handles[id])
    {
        ...
        if(err != 0)
            return (alsa_mus_error(MUS_AUDIO_CANT_CLOSE, mus_format("snd_pcm_close: %s", snd_strerror(err))));
        handles[id] = NULL;
    }
    return (MUS_NO_ERROR);
}
...
static int alsa_mus_audio_mixer_read(int ur_dev, int field, int chan, float* val)
{
    ...
    device = MUS_AUDIO_DEVICE(ur_dev);
    if(alsa_trace)
        mus_print("%s: card=%d, dev=%s<%d>, field=%s<%d>, chan=%d", c__FUNCTION__, card, mus_audio_device_name(device), device,
                  mus_audio_device_name(field), field, chan);
    ...
    if((err = to_alsa_device(device, &alsa_device, &alsa_stream)) < 0)
    {
        ...
        if(alsa_trace)
        {
            mus_print("%s: cannot translate device %s<%d> to alsa, field=%s<%d>", snd_strerror(err), mus_audio_device_name(device), device,
                      mus_audio_device_name(field), field);
        }
        return (MUS_ERROR);
    }
    if(alsa_trace)
        mus_print("%s:         adev=%d, achan=%d", c__FUNCTION__, alsa_device, alsa_stream);
    switch(field)
    {
        case MUS_AUDIO_AMP:
            /* amplitude value */
        ...
        case MUS_AUDIO_CHANNEL:
            ...
            {
                if((alsa_stream == SND_PCM_STREAM_CAPTURE) && (alsa_capture_device_name)
...
#endif
                    if((alsa_stream == SND_PCM_STREAM_CAPTURE) && (max_channels > alsa_max_capture_channels))
                    {
        ...
        case MUS_AUDIO_DIRECTION:
            ...
            break;
        default: return (alsa_mus_error(MUS_AUDIO_CANT_READ, NULL)); break;
    }
    return (MUS_NO_ERROR);
}
...
void mus_sun_set_outputs(int speakers, int headphones, int line_out)
{
    ...
    if(speakers)
        sun_default_outputs |= AUDIO_SPEAKER;
    if(headphones)
        sun_default_outputs |= AUDIO_HEADPHONE;
    if(line_out)
        sun_default_outputs |= AUDIO_LINE_OUT;
}
...
#if HAVE_SYS_MIXER_H
    if(version_name == NULL)
        version_name = (char*)CALLOC(PRINT_BUFFER_SIZE, sizeof(char));
#else
    if(version_name == NULL)
        version_name = (char*)CALLOC(LABEL_BUFFER_SIZE, sizeof(char));
#endif
#ifndef AUDIO_DEV_AMD
#if HAVE_SYS_MIXER_H
    mus_snprintf(version_name, PRINT_BUFFER_SIZE, "audio: %s (%s), %s %s %s", ad.name, ad.version, MIXER_NAME, MIXER_VERSION,
                 MIXER_CONFIGURATION);
#else
...
int mus_audio_open_output(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    if(audio_fd == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, -1, mus_format("can't open output %s: %s", dev_name, strerror(errno)));
    AUDIO_INITINFO(&info);
    ...
    if(err == -1)
    {
        ...
        if((int)info.play.channels != chans)
            RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_fd,
                              mus_format("can't set output %s (%s) channels to %d", mus_audio_device_name(dev), dev_name, chans));

        if(((int)info.play.precision != bits) || ((int)info.play.encoding != encode))
            RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, audio_fd,
                              mus_format("can't set output %s (%s) format to %d bits, %d encode (%s)", mus_audio_device_name(dev), dev_name,
                                         bits, encode, mus_data_format_name(format)));

        if((int)info.play.sample_rate != srate)
            RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_fd,
                              mus_format("can't set output %s (%s) srate to %d", mus_audio_device_name(dev), dev_name, srate));
    }
...
int mus_audio_read(int line, char* buf, int bytes)
{
    ...
    while(total < bytes)
    {
        ...
        {
            int bytes_read;
            if((total + bytes_available) > bytes)
                bytes_available = bytes - total;
...
int mus_audio_open_input(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    if(encode == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, -1,
                          mus_format("format %d bits, %d encode (%s) not available", bits, encode, mus_data_format_name(format)));
    if(getenv(AUDIODEV_ENV) != NULL)
        dev_name = getenv(AUDIODEV_ENV);
    else
        dev_name = DAC_NAME;
    if(dev != MUS_AUDIO_DUPLEX_DEFAULT)
        ...
        audio_fd = open(dev_name, O_RDWR, 0);
    if(audio_fd == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, -1, mus_format("can't open input %s: %s", dev_name, strerror(errno)));
    AUDIO_INITINFO(&info);
    /*  ioctl(audio_fd, AUDIO_GETINFO, &info); */
    ...
    if(err == -1)
        RETURN_ERROR_EXIT(
          MUS_AUDIO_CANT_OPEN, audio_fd,
          mus_format("can't set srate %d and chans %d for input %s (%s)", srate, chans, dev_name, mus_audio_device_name(dev)));
    ioctl(audio_fd, AUDIO_GETINFO, &info);
    ...
    if(err == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, audio_fd,
                          mus_format("can't set bits %d, encode %d (format %s) for input %s (%s)", bits, encode,
                                     mus_data_format_name(format), dev_name, mus_audio_device_name(dev)));
    ioctl(audio_fd, AUDIO_GETINFO, &info);
    ...
    else
    {
        if(dev == MUS_AUDIO_CD)
            ...
            indev = AUDIO_MICROPHONE;
    }
    info.record.port = indev;
    err = ioctl(audio_fd, AUDIO_SETINFO, &info);
    if(err == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_WRITE, audio_fd,
                          mus_format("can't set record.port to %d for %s (%s)", indev, dev_name, mus_audio_device_name(dev)));
    err = ioctl(audio_fd, AUDIO_GETINFO, &info);
    if(err == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, audio_fd,
                          mus_format("can't getinfo on input %s (%s, line: %d)", dev_name, mus_audio_device_name(dev), audio_fd));
    else
    {
        if((int)info.record.port != indev)
            RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, audio_fd,
                              mus_format("confusion in record.port: %d != %d (%s: %s)", (int)info.record.port, indev, dev_name,
                                         mus_audio_device_name(dev)));
        if((int)info.record.channels != chans)
            RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_fd,
                              mus_format("confusion in record.channels: %d != %d (%s: %s)", (int)info.record.channels, chans, dev_name,
                                         mus_audio_device_name(dev)));
        if(((int)info.record.precision != bits) || ((int)info.record.encoding != encode))
            RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, audio_fd,
                              mus_format("confusion in record.precision|encoding: %d != %d or %d != %d (%s: %s)",
                                         (int)info.record.precision, bits, (int)info.record.encoding, encode, dev_name,
                                         mus_audio_device_name(dev)));
    }
    ...
    if(err == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_WRITE, audio_fd,
                          mus_format("can't set buffer size to %d on input %s (%s)", size, dev_name, mus_audio_device_name(dev)));
    return (audio_fd);
}
...
int mus_audio_mixer_read(int ur_dev, int field, int chan, float* val)
{
    ...
    if(audio_fd == -1)
    {
        ...
        if(audio_fd == -1)
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, -1, mus_format("can't open %s or /dev/audioctl: %s", dev_name, strerror(errno)));
        else
            dev_name = "/dev/audioctl";
    }
    err = ioctl(audio_fd, AUDIO_GETINFO, &info);
    if(err == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, audio_fd, mus_format("can't get %s (%s) info", dev_name, mus_audio_device_name(dev)));
    if(field == MUS_AUDIO_PORT)
    {
        /* info.play|record have a field avail_ports */
        port = 1;
        if((chan > port) && (info.record.avail_ports & AUDIO_MICROPHONE))
        {
            val[port] = MUS_AUDIO_MICROPHONE;
            port++;
        }
        if((chan > port) && (info.record.avail_ports & AUDIO_LINE_IN))
        {
        ...
        if((chan > port) && (info.record.avail_ports & AUDIO_INTERNAL_CD_IN))
        {
            ...
            if(err == -1)
                RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, audio_fd,
                                  mus_format("can't get device info on %s (%s)", dev_name, mus_audio_device_name(dev)));
...
#endif
        if((chan > port) && (info.play.avail_ports & AUDIO_SPEAKER))
        {
            val[port] = MUS_AUDIO_SPEAKERS;
            port++;
        }
        if((chan > port) && (info.play.avail_ports & AUDIO_LINE_OUT))
        {
            val[port] = MUS_AUDIO_LINE_OUT;
            port++;
        }
        if((chan > port) && (info.play.avail_ports & AUDIO_HEADPHONE))
        {
    ...
    } else
    {
        if(field == MUS_AUDIO_FORMAT) /* this actually depends on the audio device */
        {
            ...
            */
            if(err == -1)
                RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, audio_fd,
                                  mus_format("can't get data format info for %s (%s)", dev_name, mus_audio_device_name(dev)));
            port = 1;
            if((ad.name) && (strcmp(ad.name, "SUNW,audio810") == 0))
            {
                val[0] = 1;
                val[1] = MUS_MULAW;
            } else
                        ...
                        case MUS_AUDIO_AMP:
                            ...
                            if(chan == 0)
                            {
                                if(info.play.balance <= (AUDIO_RIGHT_BALANCE / 2))
                                    ...
                                    val[0] = info.play.gain * (AUDIO_RIGHT_BALANCE - info.play.balance)
                                             / (float)(AUDIO_MAX_GAIN * (AUDIO_RIGHT_BALANCE / 2));
                            } else
                            {
                                if(info.play.balance >= (AUDIO_RIGHT_BALANCE / 2))
                                    val[0] = info.play.gain / (float)(AUDIO_MAX_GAIN);
                                else
                                    val[0] = info.play.gain * info.play.balance / (float)(AUDIO_MAX_GAIN * (AUDIO_RIGHT_BALANCE / 2));
                            }
                            break;
                        case MUS_AUDIO_CHANNEL:
                            val[0] = 2;
                        ...
                        case MUS_AUDIO_AMP:
                            if(chan == 0)
                            {
                                if(info.record.balance <= (AUDIO_RIGHT_BALANCE / 2))
                                    ...
                                    val[0] = info.record.gain * (AUDIO_RIGHT_BALANCE - info.record.balance)
                                             / (float)(AUDIO_MAX_GAIN * (AUDIO_RIGHT_BALANCE / 2));
                            } else
                            {
                                if(info.record.balance >= (AUDIO_RIGHT_BALANCE / 2))
                                    ...
                                    val[0] = info.record.gain * info.record.balance / (float)(AUDIO_MAX_GAIN * (AUDIO_RIGHT_BALANCE / 2));
                            }
                            break;
...
int mus_audio_mixer_write(int ur_dev, int field, int chan, float* val)
{
    ...
    if(audio_fd == -1)
    {
        ...
        if(audio_fd == -1)
            RETURN_ERROR_EXIT(
              MUS_AUDIO_CANT_WRITE, -1,
              mus_format("can't write fields of %s (or /dev/audioctl) (%s): %s", dev_name, mus_audio_device_name(dev), strerror(errno)));
        else
            dev_name = "/dev/audioctl";
    }
    err = ioctl(audio_fd, AUDIO_GETINFO, &info);
        ...
        case MUS_AUDIO_MICROPHONE:
            switch(field)
            {
                case MUS_AUDIO_AMP:
                    ...
                    break;
                case MUS_AUDIO_CHANNEL: info.record.channels = (int)val[0]; break;
                case MUS_AUDIO_SRATE: info.record.sample_rate = (int)val[0]; break;
                case MUS_AUDIO_IGAIN: info.monitor_gain = (int)(AUDIO_MAX_GAIN * val[0]); break;
                default:
                    RETURN_ERROR_EXIT(
                      MUS_AUDIO_CANT_WRITE, audio_fd,
                      mus_format("can't write %s field %d (%s)", mus_audio_device_name(dev), field, mus_audio_device_name(field)));
    ...
    if(err == -1)
        RETURN_ERROR_EXIT(
          MUS_AUDIO_CANT_WRITE, audio_fd,
          mus_format("can't write %s field %d (%s) after explicit set", mus_audio_device_name(dev), field, mus_audio_device_name(field)));
    return (mus_audio_close(audio_fd));
}
...
static char* sun_format_name(int format)
{
    switch(format)
    {
        ...
        case AUDIO_ENCODING_NONE:
            return ("not audio");
            break; /* dbri interface configured for something else */
#endif
    }
    return ("unknown");
}
...
{
    if(dev == AUDIO_MICROPHONE)
        return ("microphone");
    if(dev == AUDIO_LINE_IN)
        return ("line in");
    if(dev == AUDIO_INTERNAL_CD_IN)
        return ("cd");
    if(dev == (AUDIO_MICROPHONE | AUDIO_LINE_IN))
        return ("microphone + line in");
    if(dev == (AUDIO_MICROPHONE | AUDIO_LINE_IN | AUDIO_INTERNAL_CD_IN))
        return ("microphone + line in + cd");
    if(dev == (AUDIO_MICROPHONE | AUDIO_INTERNAL_CD_IN))
        return ("microphone + cd");
    if(dev == (AUDIO_LINE_IN | AUDIO_INTERNAL_CD_IN))
        return ("line in + cd");
    return ("unknown");
}
...
{
    if(dev == AUDIO_SPEAKER)
        return ("speakers");
    if(dev == AUDIO_LINE_OUT)
        return ("line out");
    if(dev == AUDIO_HEADPHONE)
        return ("headphones");
    if(dev == (AUDIO_SPEAKER | AUDIO_LINE_OUT))
        return ("speakers + line out");
    if(dev == (AUDIO_SPEAKER | AUDIO_LINE_OUT | AUDIO_HEADPHONE))
        return ("speakers + line out + headphones");
    if(dev == (AUDIO_SPEAKER | AUDIO_HEADPHONE))
        return ("speakers + headphones");
    if(dev == (AUDIO_LINE_OUT | AUDIO_HEADPHONE))
        return ("line out + headphones");
    return ("unknown");
}
...
{
    if(sun_vol_name == NULL)
        sun_vol_name = (char*)CALLOC(LABEL_BUFFER_SIZE, sizeof(char));
    if(chans != 2)
...
static void describe_audio_state_1(void)
{
    ...
    mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "output: %s\n    srate: %d, vol: %s, chans: %d, format %d-bit %s\n",
                 sun_out_device_name(info.play.port), info.play.sample_rate,
                 sun_volume_name((float)info.play.gain / (float)AUDIO_MAX_GAIN, info.play.balance, 2), info.play.channels,
                 info.play.precision, sun_format_name(info.play.encoding));
    pprint(audio_strbuf);
    mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "input: %s\n    srate: %d, vol: %s, chans: %d, format %d-bit %s\n",
                 sun_in_device_name(info.record.port), info.record.sample_rate,
                 sun_volume_name((float)info.record.gain / (float)AUDIO_MAX_GAIN, info.record.balance, 2), info.record.channels,
                 info.record.precision, sun_format_name(info.record.encoding));
    pprint(audio_strbuf);
    mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "input->output vol: %.3f\n", (float)info.monitor_gain / (float)AUDIO_MAX_GAIN);
    pprint(audio_strbuf);
    if(info.play.pause)
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "Playback is paused\n");
        pprint(audio_strbuf);
    }
    if(info.record.pause)
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "Recording is paused\n");
        pprint(audio_strbuf);
    }
    if(info.output_muted)
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "Output is muted\n");
        pprint(audio_strbuf);
    }
...
static void win_mus_print(char* msg)
{
    ...
    else
    {
        if(win_in_err)
            ...
            waveOutGetErrorText(win_out_err, getstr, PRINT_BUFFER_SIZE);
...
int mus_audio_open_output(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    win_out_err = waveOutOpen(&fd, WAVE_MAPPER, &wf, (DWORD)next_buffer, 0,
                              CALLBACK_FUNCTION); /* 0 here = user_data above, other case = WAVE_FORMAT_QUERY */
    if(win_out_err)
        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, mus_format("can't open %d (%s)", dev, mus_audio_device_name(dev)));
    waveOutPause(fd);
    if(size <= 0)
        buffer_size = 1024;
    else
    ...
    if((wh[0].lpData) == 0)
    {
        ...
        RETURN_ERROR_EXIT(MUS_AUDIO_SIZE_NOT_AVAILABLE,
                          mus_format("can't allocate buffer size %d for output %d (%s)", buffer_size, dev, mus_audio_device_name(dev)));
    }
    ...
    if(win_out_err)
    {
        ...
        RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE,
                          mus_format("can't setup output 'header' for %d (%s)", dev, mus_audio_device_name(dev)));
    }
...
int mus_audio_write(int line, char* buf, int bytes)
{
    ...
    start_win_print();
    if(line != OUTPUT_LINE)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_WRITE, mus_format("write error: line %d != %d?", line, OUTPUT_LINE));
    win_out_err = 0;
    leftover = bytes;
    start = 0;
    if(sound_state == SOUND_UNREADY)
    ...
    while(leftover > 0)
    {
        ...
        if(win_out_err)
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_WRITE, mus_format("write error on %d", line));
    ...
    if(devs > 0)
    {
        ...
        for(dev = 0; dev < devs; dev++)
        {
            ...
            if(!err)
            {
                ...
                if(wocaps.dwFormats & SRATE_11025_BITS)
                {
                    srate = 11025;
                    if(need_comma)
                        pprint(", ");
                ...
                if(wocaps.dwFormats & SRATE_22050_BITS)
                {
                    srate = 22050;
                    if(need_comma)
                        pprint(", ");
                ...
                if(wocaps.dwFormats & SRATE_44100_BITS)
                {
                    srate = 44100;
                    if(need_comma)
                        pprint(", ");
                ...
                if(wocaps.dwFormats & BYTE_SAMPLE_BITS)
                {
                    format = 8;
                    if(need_comma)
                        pprint(", ");
                ...
                if(wocaps.dwFormats & SHORT_SAMPLE_BITS)
                {
                    format = 16;
                    if(need_comma)
                        pprint(", ");
                    pprint(" little-endian short");
                    need_comma = 1;
                }
                if(need_comma)
                    pprint("\n");
                ...
                if(wocaps.dwSupport & WAVECAPS_VOLUME)
                {
                    ...
                    if(!err)
                    {
                        if(wocaps.dwSupport & WAVECAPS_LRVOLUME)
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "  vol: %.3f %.3f", unlog((unsigned short)(val >> 16)),
                                         unlog((unsigned short)(val & 0xffff)));
                        else
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "  vol: %.3f", unlog((unsigned short)(val & 0xffff)));
                        pprint(audio_strbuf);
                        need_comma = 1;
                    }
                }
                if(!err)
                {
                    ...
                    if(wocaps.dwSupport & WAVECAPS_PLAYBACKRATE)
                    {
                        ...
                        if(!err)
                        {
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "%s playback rate: %.3f", (need_comma ? ", " : ""),
                                         (float)rate / 65536.0);
                            pprint(audio_strbuf);
                            need_comma = 1;
                        }
                    ...
                    if(wocaps.dwSupport & WAVECAPS_PITCH)
                    {
                        ...
                        if(!err)
                        {
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "%s pitch: %.3f", (need_comma ? ", " : ""),
                                         (float)pitch / 65536.0);
                            pprint(audio_strbuf);
                            need_comma = 1;
                        }
        ...
        for(dev = 0; dev < devs; dev++)
        {
            ...
            if(!err)
            {
                ...
                if((wicaps.wMid != maker) || (version != wicaps.vDriverVersion))
                {
                    ...
                    pprint("\n");
                if(wicaps.wChannels == 2)
                    pprint("    stereo");
                else
                    pprint("    mono");
                if(wicaps.dwFormats & SRATE_11025_BITS)
                {
                    pprint(", 11025");
                    need_comma = 1;
                }
                if(wicaps.dwFormats & SRATE_22050_BITS)
                {
                    if(need_comma)
                        pprint(", ");
                    pprint(" 22050");
                ...
                if(wicaps.dwFormats & SRATE_44100_BITS)
                {
                    if(need_comma)
                        pprint(", ");
                    pprint(" 44100");
                ...
                if(wicaps.dwFormats & BYTE_SAMPLE_BITS)
                {
                    if(need_comma)
                        pprint(", ");
                    pprint(" unsigned byte");
                ...
                if(wicaps.dwFormats & SHORT_SAMPLE_BITS)
                {
                    if(need_comma)
                        pprint(", ");
                    pprint(" little-endian short");
        ...
        for(dev = 0; dev < devs; dev++)
        {
            ...
            if(!err)
            {
                ...
                if((wacaps.wMid != maker) || (version != wacaps.vDriverVersion))
                    mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, ": version %d.%d%s", (wacaps.vDriverVersion >> 8),
                                 wacaps.vDriverVersion & 0xff, (wacaps.wTechnology & AUXCAPS_CDAUDIO) ? " (CD)" : "");
                else
                ...
                if(wacaps.dwSupport & AUXCAPS_VOLUME)
                {
                    ...
                    if(!err)
                    {
                        if(wacaps.dwSupport & AUXCAPS_LRVOLUME)
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "  vol: %.3f %.3f\n", unlog((unsigned short)(val >> 16)),
                                         unlog((unsigned short)(val & 0xffff)));
                        else
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "  vol: %.3f\n", unlog((unsigned short)(val & 0xffff)));
                        pprint(audio_strbuf);
                    }
                }
            }
            ...
            if(!err)
            {
                ...
                if(!err)
                {
                    ...
                    while(happy)
                    {
                        ...
                        if(!err)
                        {
                            if((source == 0) && (!dest_time))
                                pprint("  Sources:\n");
                            if((dest == 0) && (dest_time))
                                pprint("  Destinations:\n");
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "    %s: %s (%s), %d chan%s", mixline.szName,
                                         mixer_component_name(mixline.dwComponentType), mixer_target_name(mixline.Target.dwType),

                                         mixline.cChannels, ((mixline.cChannels != 1) ? "s" : ""));
                            pprint(audio_strbuf);
                            if(mixline.cConnections > 0)
                            {
                                mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, ", %d connection%s", mixline.cConnections,
                                             ((mixline.cConnections != 1) ? "s" : ""));
                                pprint(audio_strbuf);
                            }
                            if(dest_time)
                            {
                            ...
                            if(mixline.cControls > 0)
                            {
                                ...
                                if(!err)
                                {
                                    ...
                                    for(control = 0; control < controls; control++)
                                    {
                                        ...
                                        if((ctype == MIXERCONTROL_CONTROLTYPE_EQUALIZER) || (ctype == MIXERCONTROL_CONTROLTYPE_MUX)
                                           || (ctype == MIXERCONTROL_CONTROLTYPE_MIXER) || (ctype == MIXERCONTROL_CONTROLTYPE_SINGLESELECT)
                                           || (ctype == MIXERCONTROL_CONTROLTYPE_MULTIPLESELECT))
                                        {
                                            ...
                                            err =
                                              mixerGetControlDetails((HMIXEROBJ)mfd, &controldetails, MIXER_GETCONTROLDETAILSF_LISTTEXT);
                                            if(!err)
                                            {
                                        ...
                                        if(mixline.cChannels > MAX_DESCRIBE_CHANS)
                                            ...
                                            controldetails.cChannels = mixline.cChannels;
                                        ...
                                        else
                                        {
                                            ...
                                            if(!err)
                                            {
                                                ...
                                                if(chans > MAX_DESCRIBE_CHANS)
                                                    chans = MAX_DESCRIBE_CHANS;
                                                switch(mc[control].dwControlType & MIXERCONTROL_CT_UNITS_MASK)
                                                {
                                                    case MIXERCONTROL_CT_UNITS_BOOLEAN:
...
int mus_audio_close(int line)
{
    ...
    if(line == OUTPUT_LINE)
    {
        ...
        if(sound_state != SOUND_UNREADY)
        {
            ...
            FREE(wh[1].lpData);
            if(win_out_err)
                RETURN_ERROR_EXIT(MUS_AUDIO_CANT_CLOSE, mus_format("close failed on %d", line));
        }
    } else
    {
        if(line == INPUT_LINE)
        {
            ...
            RETURN_ERROR_EXIT(MUS_AUDIO_CANT_CLOSE, mus_format("can't close unrecognized line %d", line));
    }
    return (MUS_NO_ERROR);
}
...
int mus_audio_open_input(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    if((rec_wh.lpData) == 0)
        RETURN_ERROR_EXIT(MUS_AUDIO_SIZE_NOT_AVAILABLE,
                          mus_format("can't allocated %d bytes for input buffer of %d (%s)", size, dev, mus_audio_device_name(dev)));
    win_in_err = waveInOpen(&record_fd, WAVE_MAPPER, &wf, (DWORD)next_input_buffer, 0, CALLBACK_FUNCTION);
    if(win_in_err)
    {
        FREE(rec_wh.lpData);
        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, mus_format("can't open input device %d (%s)", dev, mus_audio_device_name(dev)));
    }
    ...
    if(win_in_err)
    {
        ...
        waveInClose(record_fd);
        RETURN_ERROR_EXIT(MUS_AUDIO_CONFIGURATION_NOT_AVAILABLE,
                          mus_format("can't prepare input 'header' for %d (%s)", dev, mus_audio_device_name(dev)));
    }
    return (MUS_NO_ERROR);
}
    ...
    if(field == MUS_AUDIO_AMP)
    {
        ...
        if(!err)
        {
            if(chan == 0)
                ...
                val[0] = unlog((unsigned short)(lval & 0xffff));
            return (MUS_NO_ERROR);
        }
    }
    RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ,
                      mus_format("can't read device %d (%s) field %s", dev, mus_audio_device_name(dev), mus_audio_device_name(field)));
}
...
int mus_audio_mixer_write(int ur_dev, int field, int chan, float* val)
{
    ...
    if(field == MUS_AUDIO_AMP)
    {
        ...
        if(!err)
        {
            if(chan == 0)
                ...
                lval = (unsigned long)((lval & 0xffff0000) | ((unsigned short)(val[0] * 65535)));
    ...
    RETURN_ERROR_EXIT(MUS_AUDIO_CANT_WRITE,
                      mus_format("can't set device %d (%s) field %s", dev, mus_audio_device_name(dev), mus_audio_device_name(field)));
}
...
char* device_name(AudioDeviceID deviceID, int input_case)
{
    ...
    if(err == noErr)
        err = AudioDeviceGetPropertyInfo(deviceID, 0, false, kAudioDevicePropertyDeviceManufacturer, &msize, NULL);
    if(err == noErr)
    {
        ...
        if(trans == 0)
            ...
            mus_snprintf(full_name, size + msize + 4, "\n  %s: %s ('%c%c%c%c')", mfg, name, (char)((trans >> 24) & 0xff),
                         (char)((trans >> 16) & 0xff), (char)((trans >> 8) & 0xff), (char)(trans & 0xff));
        FREE(name);
        FREE(mfg);
    }
    return (full_name);
...
static void describe_audio_state_1(void)
{
    ...
    if(err != noErr)
        default_input = 55555555; /* unsigned int -- I want some value that won't happen! */
    size = sizeof(AudioDeviceID);
    err = AudioHardwareGetProperty(kAudioHardwarePropertyDefaultOutputDevice, &size, &default_output);
    if(err != noErr)
        default_output = 55555555;
    err = AudioHardwareGetProperty(kAudioHardwarePropertyDevices, &msize, (void*)devices);
    mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "found %d audio device%s", (int)num_devices, (num_devices != 1) ? "s" : "");
    pprint(audio_strbuf);
    for(m = 0; m < 2; m++)
    {
        for(i = 0; i < num_devices; i++)
        {
            ...
            if(err == noErr)
            {
                ...
                mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "\n    srate: %d, chans: %d", (int)(desc.mSampleRate),
                ...
                mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, ", bits/sample: %d, format: %c%c%c%c", (int)(desc.mBitsPerChannel),
                ...
                if((int)(desc.mFormatFlags) != 0) /* assuming "PCM" here */
                {
                    ...
                    pprint("\n    flags: ");
                    mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "%s%s%s%s%s%s", (flags & kLinearPCMFormatFlagIsFloat) ? "float " : "",
                ...
                if((int)(desc.mChannelsPerFrame) > 0)
                {
                    ...
                    for(j = 0; j <= (int)(desc.mChannelsPerFrame); j++)
                    {
                        ...
                        if(err == noErr)
                        {
                            mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, " %s%.3f", (j == 0) ? "master: " : "", vol);
                            pprint(audio_strbuf);
                        }
                ...
                if(err == noErr)
                {
                    ...
                    for(k = 0; k < formats; k++)
                    {
                        ...
                        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "\n      srate: %d, chans: %d, bits/sample: %d, format: %c%c%c%c",
                                     (int)(descs[k].mSampleRate), (int)(descs[k].mChannelsPerFrame), (int)(descs[k].mBitsPerChannel),
                                     (trans >> 24) & 0xff, (trans >> 16) & 0xff, (trans >> 8) & 0xff, trans & 0xff);
                        pprint(audio_strbuf);
                    }
                }
...
static OSStatus writer(AudioDeviceID inDevice, const AudioTimeStamp* inNow, const AudioBufferList* InputData,
                       const AudioTimeStamp* InputTime, AudioBufferList* OutputData, const AudioTimeStamp* OutputTime, void* appGlobals)
{
    AudioBuffer abuf;
    char *aplbuf, *sndbuf;
...
static OSStatus reader(AudioDeviceID inDevice, const AudioTimeStamp* inNow, const AudioBufferList* InputData,
                       const AudioTimeStamp* InputTime, AudioBufferList* OutputData, const AudioTimeStamp* OutputTime, void* appGlobals)
{
    AudioBuffer abuf;
    char *aplbuf, *sndbuf;
...
int mus_audio_open_output(int dev, int srate, int chans, int format, int size)
{
    ...
    if((device_desc.mChannelsPerFrame != chans) || ((int)(device_desc.mSampleRate) != srate))
    {
...
int mus_audio_read(int line, char* buf, int bytes)
{
    ...
    if(bytes <= bufsize)
        ...
        memmove((void*)buf, (void*)to_buf, bufsize);
    in_buf++;
    if(in_buf >= MAX_BUFS)
        in_buf = 0;
    return (MUS_ERROR);
...
static int max_chans(AudioDeviceID device, int input)
{
    ...
    if(err == noErr)
    {
        ...
        if(formats > 1)
        {
            ...
            if(err == noErr)
                for(k = 0; k < formats; k++)
                    if((int)(descs[k].mChannelsPerFrame) > maxc)
                        maxc = (int)(descs[k].mChannelsPerFrame);
            FREE(descs);
        }
    } else
        fprintf(stderr, "read chans hit: %s\n", osx_error(err));
    config_chans = max_chans_via_stream_configuration(device, input);
    if(config_chans > maxc)
        return (config_chans);
    return (maxc);
}
...
int mus_audio_mixer_write(int dev1, int field, int chan, float* val)
{
    ...
    switch(field)
    {
        case MUS_AUDIO_AMP:
            ...
            err =
              AudioDeviceGetPropertyInfo(dev, chan + 1, false, kAudioDevicePropertyVolumeScalar, NULL, &writable); /* "false" -> output */
            amp = (Float32)(val[0]);
...
int mus_audio_open_output(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    if((esd_format = to_esd_format(format)) == MUS_ERROR)
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, audio_out,
                          mus_format("Can't handle format %d (%s) through esd", format, mus_data_format_name(format)));
    else
        esd_prop |= esd_format;

    if(chans < 1 || chans > 2)
        RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_out, mus_format("Can't handle format %d channels through esd", format));
    else
        esd_prop |= chans == 1 ? ESD_MONO : ESD_STEREO;

    esd_play_sock = esd_play_stream(esd_prop, srate, NULL, "snd playback stream");

    if(esd_play_sock == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, audio_out,
                          mus_format("Sonorus device %d (%s) not available", ur_dev, mus_audio_device_name(ur_dev)));
    else
...
{
    esd_close(line);
    if(esd_play_sock == line)
        esd_play_sock = -1;
    else if(esd_rec_sock == line)
        esd_rec_sock = -1;
    return MUS_NO_ERROR;
}
...
int mus_audio_read(int line, char* buf, int bytes)
{
    ...
    do
    {
        ...
        if(bytes_read > 0)
        { /* 0 -> EOF; we'll regard that as an error */
            bytes -= bytes_read;
            buf += bytes_read;
        } else
            RETURN_ERROR_EXIT(MUS_AUDIO_WRITE_ERROR, -1, mus_format("read error: %s", strerror(errno)));
    } while(bytes > 0);
    return MUS_NO_ERROR;
}
...
int mus_audio_open_input(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    if(chans < 1 || chans > 2)
        RETURN_ERROR_EXIT(MUS_AUDIO_CHANNELS_NOT_AVAILABLE, audio_out, mus_format("Can't handle format %d channels through esd", chans));
    else
        esd_prop |= chans == 1 ? ESD_MONO : ESD_STEREO;

    esd_rec_sock = esd_play_stream(esd_prop, srate, NULL, "snd record stream");

    if(esd_rec_sock == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_DEVICE_NOT_AVAILABLE, audio_out,
                          mus_format("Device %d (%s) not available", ur_dev, mus_audio_device_name(ur_dev)));
    else
        return esd_rec_sock;
}
...
int mus_audio_mixer_read(int ur_dev, int field, int chan, float* val)
{
    ...
    switch(field)
    {
        case MUS_AUDIO_AMP:
            ...
            val[0] = 1.0;
            break;
        case MUS_AUDIO_SAMPLES_PER_CHANNEL: val[0] = 44100; break;
        case MUS_AUDIO_CHANNEL:
            /* number of channels */
            val[0] = 2.0;
            if(chan > 1)
...
static void sndjack_read_process(jack_nframes_t nframes)
{
    ...
    if(sndjack_num_read_channels_inuse == 0)
        return;
...
{
    int i, ch;
    int nframes = bytes / sndjack_read_format == MUS_COMP_FLOAT ? sizeof(float) : sndjack_read_format == MUS_COMP_SHORT ? sizeof(short) : 1;
    float* buf_f = buf;
    short* buf_s = buf;
    char* buf_c = buf;
    ...
    for(i = 0; i < nframes; i++)
    {
        ...
        for(ch = 0; ch < chs; ch++)
        {
            ...
            {
                case MUS_BYTE: buf_c[i * chs + ch] = sndjack_read_channels[ch].buffer[sj_r_readplace] * 127.9f; break;
                case MUS_COMP_SHORT: buf_s[i * chs + ch] = sndjack_read_channels[ch].buffer[sj_r_readplace] * 32767.9f; break;
                case MUS_COMP_FLOAT: buf_f[i * chs + ch] = sndjack_read_channels[ch].buffer[sj_r_readplace]; break;
            }
        }
        sj_r_unread--;
        sj_r_readplace++;
...
static void sndjack_write(sample_t** buf, int nframes, int latencyframes, int chs)
{
    ...
    for(i = 0; i < nframes; i++)
    {
        while(sj_status == SJ_RUNNING && (sj_unread == sj_buffersize || sj_unread >= SJ_MAX(sj_jackbuffersize * 2, latencyframes)))
        {
            jack_mus_watchdog_counter++;
...
static int sndjack_init(void)
{
    ...
    for(ch = 0; ch < numch; ch++)
    {
        ...
        if((sndjack_channels[ch].port = jack_port_register(sndjack_client, strdup(temp), JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput, 0))
           == NULL)
        {
    ...
    for(ch = 0; ch < sndjack_num_read_channels_allocated; ch++)
    {
        ...
        if((sndjack_read_channels[ch].port = jack_port_register(sndjack_client, strdup(temp), JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0))
           == NULL)
        {
            fprintf(stderr, "Error. Could not register jack port.\n");
            goto failed_register;
...

    {
        ...
        {
            if(jack_connect(sndjack_client, jack_port_name(sndjack_channels[ch].port), outportnames[ch]))
            {
...

    {
        ...
        {
            if(jack_connect(sndjack_client, inportnames[ch], jack_port_name(sndjack_read_channels[ch].port)))
            {
...
int jack_mus_audio_open_output(int dev, int srate, int chans, int format, int size)
{
    ...
    {
        int lokke;
        // printf("Warning, sample-rate differs between snd and jack. Sound will not be played correctly!
...
int jack_mus_audio_write(int line, char* buf, int bytes)
{
    ...
    for(ch = 0; ch < sndjack_num_channels_inuse; ch++)
    {
        ...
        {
            case MUS_BYTE: len = sndjack_from_byte(ch, sndjack_num_channels_inuse, buf, buf2, bytes); break;
            case MUS_COMP_SHORT: len = sndjack_from_short(ch, sndjack_num_channels_inuse, (short*)buf, buf2, bytes); break;
            case MUS_COMP_FLOAT: len = sndjack_from_float(ch, sndjack_num_channels_inuse, (float*)buf, buf2, bytes); break;
        }
        if(len < 0)
        {
        ...
        {
            SRC_DATA src_data = {buf2, sndjack_buffer[ch], len, SNDJACK_BUFFERSIZE, 0, 0, 0, sndjack_srcratio};
...
int jack_mus_audio_mixer_read(int dev, int field, int chan, float* val)
{
    ...
    switch(field)
    {
        ...
        case MUS_AUDIO_PORT:
            ...
            break;
        case MUS_AUDIO_CHANNEL: val[0] = sndjack_num_read_channels_allocated; break;
        case MUS_AUDIO_AMP: val[0] = 1.0f; break;
        default: printf("Got unknown request with field %d %d\n", field, MUS_AUDIO_AMP); return MUS_ERROR;
    }
...
static void jack_describe_audio_state_1(void)
{
    ...
    pprint(temp);
    sprintf(temp, "\tCurrent cpu-load: %f\n", jack_cpu_load(sndjack_client));
    pprint(temp);
    sprintf(temp, "\tIs running realtime: %s\n", jack_is_realtime(sndjack_client) == 1 ? "yes" : "no");
    pprint(temp);
    sprintf(temp, "\tResample quality (only used when needed): %s (%s)\n", src_get_name(SRC_QUALITY), src_get_description(SRC_QUALITY));
    pprint(temp);
    sprintf(temp, "\tIs able to handle the following audio formats: %s %s %s\n", mus_data_format_to_string(MUS_BYTE),
            mus_data_format_to_string(MUS_COMP_SHORT), mus_data_format_to_string(MUS_COMP_FLOAT));
    pprint(temp);
    sprintf(temp, "\tPrefered audio format: %s\n", mus_data_format_to_string(MUS_COMP_FLOAT));
    pprint(temp);
}
...
int mus_audio_open_output(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    if(fd == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, -1, mus_format("can't open /dev/audio for output: %s", strerror(errno)));
    ioctl(fd, AUDIO_SET_CHANNELS, chans);
    if(dev == MUS_AUDIO_SPEAKERS)
        ioctl(fd, AUDIO_SET_OUTPUT, AUDIO_OUT_SPEAKER);
    else if(dev == MUS_AUDIO_LINE_OUT)
        ioctl(fd, AUDIO_SET_OUTPUT, AUDIO_OUT_LINE);
    else
        ioctl(fd, AUDIO_SET_OUTPUT, AUDIO_OUT_HEADPHONE);
    ...
    else if(format == MUS_ALAW)
        ioctl(fd, AUDIO_SET_DATA_FORMAT, AUDIO_FORMAT_ALAW);
    else
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, fd,
                          mus_format("can't set output format to %d (%s) for %d (%s)", format, mus_audio_format_name(format), dev,
                                     mus_audio_device_name(dev)));
    ...
    for(i = 0; i < desc.nrates; i++)
        if(srate == desc.sample_rate[i])
            break;
    if(i == desc.nrates)
        RETURN_ERROR_EXIT(SRATE_NOT_AVAILABLE, fd, mus_format("can't set srate to %d on %d (%s)", srate, dev, mus_audio_device_name(dev)));
...
int mus_audio_mixer_read(int ur_dev, int field, int chan, float* val)
{
    ...
    if(field == MUS_AUDIO_PORT)
    {
        ...
        if(chan > 2)
            val[2] = MUS_AUDIO_DAC_OUT;
        if(chan > 3)
            val[3] = MUS_AUDIO_LINE_OUT;
        if(chan > 4)
            val[4] = MUS_AUDIO_LINE_IN;
    } else
    {
    ...
    if(err == MUS_ERROR)
        RETURN_ERROR_EXIT(
          MUS_AUDIO_CANT_READ, audio_fd,
          mus_format("can't read %s field of device %d (%s)", mus_audio_device_name(field), dev, mus_audio_device_name(dev)));
...
int mus_audio_mixer_write(int ur_dev, int field, int chan, float* val)
{
    ...
    if(err == MUS_ERROR)
        RETURN_ERROR_EXIT(
          MUS_AUDIO_CANT_READ, audio_fd,
          mus_format("can't set %s field of device %d (%s)", mus_audio_device_name(field), dev, mus_audio_device_name(dev)));
...
int mus_audio_open_input(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    fd = open("/dev/audio", O_RDWR);
    if(fd == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, NULL, mus_format("can't open /dev/audio for input: %s", strerror(errno)));
    ioctl(fd, AUDIO_SET_CHANNELS, chans);
    if(dev == MUS_AUDIO_MICROPHONE)
        ioctl(fd, AUDIO_SET_INPUT, AUDIO_IN_MIKE);
    else
        ioctl(fd, AUDIO_SET_INPUT, AUDIO_IN_LINE);
    if(format == MUS_BSHORT)
        ioctl(fd, AUDIO_SET_DATA_FORMAT, AUDIO_FORMAT_LINEAR16BIT);
    else if(format == MUS_MULAW)
        ioctl(fd, AUDIO_SET_DATA_FORMAT, AUDIO_FORMAT_ULAW);
        ...
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, fd,
                          mus_format("can't set input format to %d (%s) on %d (%s)", format, mus_audio_format_name(format), dev,
                                     mus_audio_device_name(dev)));
    ioctl(fd, AUDIO_DESCRIBE, &desc);
    for(i = 0; i < desc.nrates; i++)
        if(srate == desc.sample_rate[i])
    ...
    if(i == desc.nrates)
        RETURN_ERROR_EXIT(MUS_AUDIO_SRATE_NOT_AVAILABLE, fd,
                          mus_format("can't set srate to %d on %d (%s)", srate, dev, mus_audio_device_name(dev)));
    ioctl(fd, AUDIO_SET_SAMPLE_RATE, srate);
    return (fd);
}
...
static int bsd_format_to_sndlib(int encoding)
{
    switch(encoding)
    {
        ...
        case AUDIO_ENCODING_LINEAR:
            return (MUS_BSHORT);
            break; /* "sun compatible" so probably big-endian? */
...
void mus_netbsd_set_outputs(int speakers, int headphones, int line_out)
{
    ...
    if(speakers)
        netbsd_default_outputs |= AUDIO_SPEAKER;
    if(headphones)
        netbsd_default_outputs |= AUDIO_HEADPHONE;
    if(line_out)
...
int mus_audio_open_output(int dev, int srate, int chans, int format, int size)
{
    ...
    else
    {
        if(dev == MUS_AUDIO_SPEAKERS)
            ...
            a_info.play.port = netbsd_default_outputs;
    }
...
static void describe_audio_state_1(void)
{
    ...
    if(err == 0)
    {
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "  play: srate: %d, chans: %d, format: %s (%d bits), ", a_info.play.sample_rate,
                     a_info.play.channels, mus_data_format_short_name(bsd_format_to_sndlib(a_info.play.encoding)), a_info.play.precision);
        pprint(audio_strbuf);
        ...
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "volume: %.3f %.3f (gain: %d, balance: %d)\n",
                     ...
                     amp * ((float)(a_info.play.balance) / (float)(2 * AUDIO_MID_BALANCE)), a_info.play.gain, a_info.play.balance);
        pprint(audio_strbuf);

        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "  record: srate: %d, chans: %d, format: %s (%d bits), ", a_info.record.sample_rate,
                     a_info.record.channels, mus_data_format_short_name(bsd_format_to_sndlib(a_info.record.encoding)),
        ...
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "volume: %.3f %.3f (gain: %d, balance: %d)\n",
                     ...
                     amp * ((float)(a_info.record.balance) / (float)(2 * AUDIO_MID_BALANCE)), a_info.record.gain, a_info.record.balance);
        pprint(audio_strbuf);
    }
    ...
    for(i = 0;; i++)
    {
        ...
        mus_snprintf(audio_strbuf, PRINT_BUFFER_SIZE, "    %s (%s, bits: %d)\n",
                     mus_data_format_short_name(bsd_format_to_sndlib(e_info.encoding)), e_info.name, e_info.precision);
        pprint(audio_strbuf);
    }
                    ...
                    switch(field)
                    {
                        case MUS_AUDIO_AMP:
                        {
                            ...
                            if(chan == 0)
                                ...
                                val[0] = amp * ((float)(info.play.balance) / (float)(2 * AUDIO_MID_BALANCE));
                        }
                        break;
                        case MUS_AUDIO_CHANNEL: val[0] = 2; break;
                        case MUS_AUDIO_SRATE: val[0] = (float)info.play.sample_rate; break;
                    ...
                    switch(field)
                    {
                        case MUS_AUDIO_AMP:
                        {
                            ...
                            if(chan == 0)
                                ...
                                val[0] = amp * ((float)(info.record.balance) / (float)(2 * AUDIO_MID_BALANCE));
                        }
                        break;
                        case MUS_AUDIO_CHANNEL: val[0] = 1; break;
                        case MUS_AUDIO_SRATE: val[0] = (float)(info.record.sample_rate); break;
...
int mus_audio_mixer_write(int ur_dev, int field, int chan, float* val)
{
    ...
    if(audio_fd == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_WRITE, -1, mus_format("can't open /dev/sound: %s", strerror(errno)));

    err = ioctl(audio_fd, AUDIO_GETINFO, &info);
    if(err == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_READ, audio_fd, mus_format("can't get /dev/sound info"));
...
int mus_audio_open_input(int ur_dev, int srate, int chans, int format, int size)
{
    ...
    if(encode == AUDIO_ENCODING_NONE)
        RETURN_ERROR_EXIT(MUS_AUDIO_FORMAT_NOT_AVAILABLE, -1,
                          mus_format("format %s not available for recording", mus_data_format_name(format)));

    if(dev != MUS_AUDIO_DUPLEX_DEFAULT)
        audio_fd = open("/dev/sound", O_RDONLY, 0);
    else
        audio_fd = open("/dev/sound", O_RDWR, 0);
    if(audio_fd == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_OPEN, -1, mus_format("can't open /dev/sound: %s", strerror(errno)));
    ...
    err = ioctl(audio_fd, AUDIO_SETINFO, &info);
    if(err == -1)
        RETURN_ERROR_EXIT(MUS_AUDIO_CANT_WRITE, audio_fd, mus_format("can't set up for recording"));
    return (audio_fd);
}
...
char* mus_audio_report(void)
{
    ...
    print_it = 0;
    if(!audio_strbuf)
        audio_strbuf = (char*)CALLOC(PRINT_BUFFER_SIZE, sizeof(char));
...
int mus_audio_compatible_format(int dev)
{
    ...
    if(err != MUS_ERROR)
    {
        ...
        for(i = 1; i <= ival[0]; i++)
            if((ival[i] == MUS_BINT) || (ival[i] == MUS_LINT) || (ival[i] == MUS_BFLOAT) || (ival[i] == MUS_LFLOAT)
               || (ival[i] == MUS_BSHORT) || (ival[i] == MUS_LSHORT))
                return (ival[i]);
        for(i = 1; i <= ival[0]; i++)
            if((ival[i] == MUS_MULAW) || (ival[i] == MUS_ALAW) || (ival[i] == MUS_UBYTE) || (ival[i] == MUS_BYTE))
                return (ival[i]);
        return (ival[1]);
    }
#endif
