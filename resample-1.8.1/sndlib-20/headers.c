 ...
 * int mus_header_write (const char *name, int type, int in_srate, int in_chans, off_t loc, off_t size_in_samples, int format, const char
 * *comment, int len) int mus_header_initialize (void)
 *
...
#define HDRBUFSIZ 256u
static unsigned char* hdrbuf;
#define INITIAL_READ_SIZE 32u
...
int mus_header_initialize(void)
{
    if(!hdrbuf_is_inited)
    {
        ...
        hdrbuf = (unsigned char*)CALLOC(HDRBUFSIZ, sizeof(char));
        ...
        if((hdrbuf == NULL) || (aux_comment_start == NULL) || (aux_comment_end == NULL) || (loop_modes == NULL) || (loop_starts == NULL)
           || (loop_ends == NULL))
...
static const unsigned char I_SOUN[4] = {'S', 'O', 'U',
                                        'N'}; /* Sound Tools first word="SOUND" -- not unique as SMP files start with "SOUND SAMPLE" */
...
static unsigned chans = 0;
static int srate = 0, header_type = MUS_UNSUPPORTED, data_format = MUS_UNKNOWN, original_data_format = 0;
...
{
    if(aux_comment_start)
        return (aux_comment_start[n]);
    else
        return (-1);
}
...
unsigned mus_bytes_per_sample(int format)
{
    switch(format)
    {
        ...
        default:
            return (1);
            break; /* we divide by this number, so 0 is not safe */
    }
}
...
{
    return size / (off_t)mus_bytes_per_sample(format);
}
...
static const char* any_data_format_name(int sndlib_format)
{
    if(MUS_DATA_FORMAT_OK(sndlib_format))
        ...
        return (mus_header_original_format_name(mus_header_original_format(), mus_header_type()));
}
...
static int read_next_header(const char* filename, int chan)
{
    ...
    if(data_location < 24)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s: data location: %d?", filename, (int)data_location));
    ...
    {
        if((unsigned long)true_file_length > (unsigned long)(1ul << 31))
    ...
    switch(original_data_format)
    {
        ...
        case 2:
            ...
            break; /* some sound files assume MUS_UBYTE here! (NAS from 1994 X11R6 contrib) */
        ...
        case 18:
            ...
            break; /* "emphasized": Xavier Serra's de-emphasis filter: y(n) = x(n) + .9 y(n-1) */
        case 27: data_format = MUS_ALAW; break;
        case 30:
            data_format = MUS_LINT;
            break; /* from here on, for Snd's internal benefit -- these are probably not used elsewhere */
    ...
    chans = (unsigned)mus_char_to_bint((unsigned char*)(hdrbuf + 20));
    comment_start = 0;
    comment_end = 0;
    for(i = 24; i < (int)data_location - 1; i++)
        if(hdrbuf[i] == '\0')
            break;
        else
        {
            ...
            {
                comment_start = i;
                comment_end = (int)data_location - 1;
                break;
            }
        }
    ...
    if(maybe_bicsf == 107364)
        err = read_bicsf_header(filename, chan);
...
static int sndlib_format_to_next(int format)
{
    switch(format)
    {
        ...
        default:
            return (mus_error(MUS_UNSUPPORTED_DATA_FORMAT, "Next header: can't write data format: %d (%s)", format,
...
#define CHK_READ(Fd, Buf, Len)                                 \
    do                                                         \
    {                                                          \
        if(((Len) > 0) && (read(Fd, Buf, (size_t)(Len)) == 0)) \
...
{
    if(len > 0)
        CHK_WRITE(fd, (const unsigned char*)comment, (unsigned)len);
    len = loc - (len + 24);
    if(len > 0)
    {
        unsigned char* combuf;
        combuf = (unsigned char*)CALLOC(len, sizeof(char));
        CHK_WRITE(fd, combuf, (unsigned)len);
        FREE(combuf);
    }
}
...
int mus_header_write_next_header(int chan, int wsrate, unsigned wchans, int loc, int siz, int format, const char* comment, int len)
{
    ...
    mus_bint_to_char((unsigned char*)(hdrbuf + 20), (int)wchans);
...
static size_t update_form_size;
static off_t update_frames_location, update_ssnd_location;

static int seek_and_read(int chan, unsigned char* buf, off_t offset, unsigned nbytes)
{
    if(offset < 0)
        return (-1);
...
static int read_aiff_header(const char* filename, int chan, int overall_offset)
{
    ...
    int i, j = 0;
    unsigned chunksize, chunkloc, ssnd_bytes;
    ...
    chunkloc = (unsigned)(12 + overall_offset);
    offset = 0;
    for(i = 0; i < AUX_COMMENTS; i++)
        aux_comment_start[i] = 0;
    data_format = MUS_BSHORT;
    srate = 0;
    chans = 0;
    happy = true;
    true_file_length = SEEK_FILE_LENGTH(chan);
    update_form_size =
      mus_char_to_ubint((unsigned char*)(hdrbuf + 4 + overall_offset)); /* should be file-size-8 unless there are multiple forms */
    while(happy)
    {
        offset += chunkloc;
        if(seek_and_read(chan, (unsigned char*)hdrbuf, (off_t)offset, 32) <= 0)
        {
            if((got_comm) && (data_location > 0))
            {
                mus_print("%s, aiff header: chunks confused at %d; will try to continue", filename, (int)offset);
                break;
            }
            return (mus_error(MUS_HEADER_READ_FAILED, "%s, aiff header: chunks confused at %d", filename, (int)offset));
        }
        ...
        if(match_four_chars((unsigned char*)hdrbuf, I_COMM))
        {
            unsigned frames;
            got_comm = true;
            chans = mus_char_to_ubshort((unsigned char*)(hdrbuf + 8));
            ...
            if(original_data_format == 8)
                data_format = MUS_BYTE;
            else if(original_data_format == 16)
                data_format = MUS_BSHORT;
            else if(original_data_format == 24)
                data_format = MUS_B24INT;
            else if(original_data_format == 32)
                data_format = MUS_BINT;
            else if(original_data_format == 64)
                data_format = MUS_BDOUBLE;
            else
                return (mus_error(MUS_HEADER_READ_FAILED, "%s: bits per sample: %d?", filename,
                                  mus_char_to_bshort((unsigned char*)(hdrbuf + 14))));
            ...
            if(type_specifier == mus_char_to_uninterpreted_int((unsigned const char*)I_AIFC))
            {
                ...
                if(chunksize == 18)
                    chunksize += (unsigned)(5 + hdrbuf[30]); /* 5 = chunk header length in this case */
                    ...
                    else
                    {
                        ...
                        else
                        {
                               ...
                               || (match_four_chars((unsigned char*)(hdrbuf + 26), I_ni23)))
                            {
                                ...
                                if(data_format == MUS_BSHORT)
                                    data_format = MUS_LSHORT;
                                else if(data_format == MUS_B24INT)
                                    data_format = MUS_L24INT;
                                else if(data_format == MUS_BINT)
                                    data_format = MUS_LINT;
                            } else
                            {
                                if(match_four_chars((unsigned char*)(hdrbuf + 26), I_raw_))
                                {
                                    if(data_format == MUS_BYTE)
                                        data_format = MUS_UBYTE;
                                    else if(data_format == MUS_BSHORT)
        ...
        } else
        {
            ...
            {
                if(data_location != 0)
                    return (mus_error(MUS_HEADER_READ_FAILED, "%s: two SSND chunks found", filename));
                ...
                if(chunksize == 0)
                    break; /* this may happen while pre-reading an in-progress output file for updating */
            ...
            } else
            {
                if((match_four_chars((unsigned char*)hdrbuf, I_ANNO)) || (match_four_chars((unsigned char*)hdrbuf, I_COMT))
                   || (match_four_chars((unsigned char*)hdrbuf, I_NAME)) || (match_four_chars((unsigned char*)hdrbuf, I_AUTH)))
                {
                    j = 0;
                    for(i = 0; i < AUX_COMMENTS; i++)
                    ...
                    } else
                    {
                        ...
                        } else
                        {
                            if(match_four_chars((unsigned char*)hdrbuf, I_MARK))
                            {
                                ...
                                {
                                    if(seek_and_read(chan, (unsigned char*)hdrbuf, (off_t)offset + moff, 8) > 0)
                                    {
...
static int sndlib_format_to_aiff_bits(int format)
{
    switch(format)
    {
        ...
        case MUS_LSHORT:
        case MUS_UBSHORT:
        case MUS_ULSHORT: return (16); break;
        case MUS_B24INT:
        case MUS_L24INT: return (24); break;
        case MUS_BINT:
        case MUS_LINT:
        case MUS_BFLOAT:
        case MUS_LFLOAT: return (32); break;
        case MUS_BDOUBLE:
        case MUS_LDOUBLE: return (64); break;
        case MUS_BYTE:
        case MUS_UBYTE:
        case MUS_MULAW:
        case MUS_ALAW: return (8); break;
        default:
            return (mus_error(MUS_UNSUPPORTED_DATA_FORMAT, "aiff header: can't write data format: %d (%s)", format,
                              any_data_format_name(format)));
            break;
    }
}
...
static const char* sndlib_format_to_aifc_name(int format)
{
    switch(format)
    {
        ...
        case MUS_BINT:
        case MUS_BYTE:
            return ((const char*)I_NONE);
            break; /* use in24 and in32? */
        case MUS_LSHORT:
        case MUS_L24INT:
        case MUS_LINT:
            return ((const char*)I_sowt);
            break; /* should this use ni23? */
        case MUS_BFLOAT: return ((const char*)I_fl32); break;
        case MUS_BDOUBLE: return ((const char*)I_fl64); break;
        case MUS_UBYTE:
        case MUS_UBSHORT: return ((const char*)I_raw_); break;
...
static int write_aif_header(int chan, int wsrate, unsigned wchans, unsigned siz, int format, const char* comment, int len, bool aifc_header)
{
    ...
    unsigned i, curend;
    int j, lenhdr, lenloop, extra; /* set aifc to 0 to get old-style AIFF header */
    lenhdr = 0;
    extra = 0;
    curend = 0;
    lenloop = 38;
    ...
    if(aifc_header)
        mus_bint_to_char((unsigned char*)(hdrbuf + 4), len + 30 + 16 + lenloop + (int)siz + lenhdr + extra + 12 + 10);
    else
        mus_bint_to_char((unsigned char*)(hdrbuf + 4), len + 30 + 16 + lenloop + (int)siz + lenhdr + extra);
    /*
    ...
    if(aifc_header)
    {
        ...
        mus_ubint_to_char((unsigned char*)(hdrbuf + 8), 0xA2805140);
    } else
        write_four_chars((unsigned char*)(hdrbuf + 8), I_AIFF);
    write_four_chars((unsigned char*)(hdrbuf + 12), I_COMM);
    if(aifc_header)
        mus_bint_to_char((unsigned char*)(hdrbuf + 16), 18 + 10);
    else
        mus_bint_to_char((unsigned char*)(hdrbuf + 16), 18);
    mus_bshort_to_char((unsigned char*)(hdrbuf + 20), (short)wchans);
    if(wchans > 0)
        mus_ubint_to_char((unsigned char*)(hdrbuf + 22), siz / ((unsigned)wchans * mus_bytes_per_sample(format)));
    ...
    {
        const char* str;
        str = sndlib_format_to_aifc_name(format);
    ...
    if(len != 0)
    {
        ...
        for(j = 0; j < len; j++)
        {
            ...
            hdrbuf[i] = (unsigned char)comment[j];
            i++;
        }
        if(extra != 0)
        {
            if((i + (unsigned)extra) > HDRBUFSIZ)
            {
                curend += i;
    ...
    mus_ubint_to_char((unsigned char*)(hdrbuf + 4), siz + 8);
        ...
        if(full_len > 0)
        {
            ...
            for(i = 0; i < AUX_COMMENTS; i++)
            {
                ...
                if((start > 0) && (start < end))
                {
                    ...
                    CHK_READ(fd, (char*)(sc + sc_len), (unsigned)len);
                    for(j = 0; j < len; j++)
                        if(sc[j + sc_len] == 0)
                            sc[j + sc_len] = ' ';
                    sc_len += len;
...
static int read_riff_header(const char* filename, int chan)
{
    ...
    unsigned chunksize, chunkloc, i;
    bool little, got_fmt = false;
    off_t offset;
    little = true;
    if(match_four_chars((unsigned char*)hdrbuf, I_RIFX))
        little = false; /* big-endian data in this case, but I've never seen one */
    ...
    for(i = 0; i < AUX_COMMENTS; i++)
        aux_comment_start[i] = 0;
    true_file_length = SEEK_FILE_LENGTH(chan);
    update_form_size = big_or_little_endian_uint((unsigned char*)(hdrbuf + 4), little);
    while(true)
    {
        offset += chunkloc;
        if(offset >= true_file_length)
            break;
        if(seek_and_read(chan, (unsigned char*)hdrbuf, (off_t)offset, 64) <= 0)
            break;
        ...
        if(match_four_chars((unsigned char*)hdrbuf, I_fmt_))
        {
            ...
            chans = (unsigned)big_or_little_endian_short((unsigned char*)(hdrbuf + 10), little);
                ...
                } else
                {
                    ...
                    } else
                    {
                        ...
                        } else
                        {
                            ...
                            {
                                aux_comment_start[0] = (off_t)offset + 8;
                                aux_comment_end[0] = (off_t)(offset + 8 + chunksize - 1);
                            }
                        }
                    }
                }
...
static int write_riff_header(int chan, int wsrate, unsigned wchans, int siz, int format, const char* comment, int len)
{
    unsigned i, extra;
    int j, lenhdr;
    size_t offset, curend;
    lenhdr = 0;
    extra = 0;
    if(len != 0)
    {
    ...
    mus_lint_to_char((unsigned char*)(hdrbuf + 4), len + 36 + siz + lenhdr + (int)extra);
    ...
    switch(format)
    {
        ...
        case MUS_LDOUBLE:
            ...
            break;
        default:
            return (mus_error(MUS_UNSUPPORTED_DATA_FORMAT, "riff header: can't write data format: %d (%s)", format,
                              any_data_format_name(format)));
            break;
    }
    mus_lshort_to_char((unsigned char*)(hdrbuf + 22), (short)wchans);
    mus_lint_to_char((unsigned char*)(hdrbuf + 24), wsrate);
    mus_ulint_to_char((unsigned char*)(hdrbuf + 28), (unsigned)wsrate * wchans * mus_bytes_per_sample(format)); /* added chans 10-Mar-99 */
    mus_ulshort_to_char((unsigned char*)(hdrbuf + 32), (unsigned short)(wchans * mus_bytes_per_sample(format)));
    ...
    if(len != 0)
    {
        offset += (unsigned)len + 12;
        write_four_chars((unsigned char*)(hdrbuf + 36), I_clm_);
        mus_lint_to_char((unsigned char*)(hdrbuf + 40), len + (int)extra);
        i = 44;
        for(j = 0; j < len; j++)
        {
            if(i == HDRBUFSIZ)
...
char* mus_header_riff_aux_comment(const char* name, off_t* starts, off_t* ends)
{
    ...
    if((starts) && (starts[0] != 0))
    {
        ...
        CHK_READ(fd, auxcom, (unsigned)(end - i + 1));
        CLOSE(fd, name);
        if(!auxcom)
            return NULL;
        i += 4;
...
static int read_soundforge_header(const char* filename, int chan)
{
    ...
    unsigned chunksize, chunkloc, i, off;
    off_t offset;
    chunkloc = 12 * 2 + 16;
    offset = 0;
    data_format = MUS_UNKNOWN;
    ...
    for(i = 0; i < AUX_COMMENTS; i++)
        aux_comment_start[i] = 0;
    true_file_length = SEEK_FILE_LENGTH(chan);
    update_form_size = mus_char_to_ulint((unsigned char*)(hdrbuf + 4 * 2));
    while(true)
    {
        offset += chunkloc;
        if(offset >= true_file_length)
        ...
        if(match_four_chars((unsigned char*)hdrbuf, I_fmt_))
        {
            ...
            chans = mus_char_to_ulshort((unsigned char*)(hdrbuf + 10 + off));
...
static int read_avi_header(const char* filename, int chan)
{
    ...
    while(happy)
    {
        ...
        if(seek_and_read(chan, (unsigned char*)hdrbuf, offset, 32) <= 0)
            return (mus_error(MUS_HEADER_READ_FAILED, "%s avi header: chunks confused at %d", filename, (int)offset));
                        ...
                        while(cktotalr < cksize)
                        {
                            ...
                            if(match_four_chars((unsigned char*)hdrbuf, I_strf))
                            {
                                if(cksizer < HDRBUFSIZ)
                                    ...
                                    rdsize = HDRBUFSIZ;
                                CHK_READ(chan, hdrbuf, (unsigned)rdsize);
                                original_data_format = mus_char_to_lshort((unsigned char*)hdrbuf);
                                chans = mus_char_to_ulshort((unsigned char*)(hdrbuf + 2));
    ...
    if(data_location > true_file_length)
        return (
          mus_error(MUS_HEADER_READ_FAILED, "%s: data_location %d > file length: %d", filename, (int)data_location, (int)true_file_length));
...
static void soundfont_entry(const char* name, int start, int end, int loop_start, int loop_end)
{
    if(soundfont_entries == soundfont_size)
    {
        ...
        for(i = soundfont_entries; i < soundfont_size; i++)
            soundfont_names[i] = NULL;
    }
    if(soundfont_names[soundfont_entries] == NULL)
        soundfont_names[soundfont_entries] = (char*)CALLOC(20, sizeof(char));
...
{
    return (int)(soundfont_entries);
}
char* mus_header_sf2_name(int n)
{
    return (soundfont_names[n]);
}
int mus_header_sf2_start(int n)
{
    return (soundfont_starts[n]);
}
int mus_header_sf2_end(int n)
{
    return (soundfont_ends[n]);
}
int mus_header_sf2_loop_start(int n)
{
    return (soundfont_loop_starts[n]);
}
int mus_header_sf2_loop_end(int n)
{
    return (soundfont_loop_ends[n]);
}
...
static int read_soundfont_header(const char* filename, int chan)
{
    ...
    while(happy)
    {
        offset += chunkloc;
        if(seek_and_read(chan, (unsigned char*)hdrbuf, offset, 32) <= 0)
            return (mus_error(MUS_HEADER_READ_FAILED, "%s soundfont header: chunks confused at %d", filename, (int)offset));
        chunksize = mus_char_to_lint((unsigned char*)(hdrbuf + 4));
        if((chunksize == 0) && /* can be empty data chunk? */
           (hdrbuf[0] == 0) && (hdrbuf[1] == 0) && (hdrbuf[2] == 0) && (hdrbuf[3] == 0))
            break;
                ...
                while(srate == 0)
                {
                    ...
                    if(match_four_chars((unsigned char*)hdrbuf, I_shdr))
                    {
                        ...
                        while(i < cksize)
                        {
                            ...
                            if((type == 1) && (mus_char_to_lint((unsigned char*)(hdrbuf + 24)) > 0))
                            {
                                if(srate == 0)
                                    srate = mus_char_to_lint((unsigned char*)(hdrbuf + 36));
                                soundfont_entry((char*)(hdrbuf), mus_char_to_lint((unsigned char*)(hdrbuf + 20)),
...
static int decode_nist_value(char* str, int base, int end)
{
    ...
    i = base;
    while((i < end) && (i < MAX_FIELD_LENGTH) && (str[i] != '-'))
        i++; /* look for -i or whatever */
    while((i < end) && (i < MAX_FIELD_LENGTH) && (str[i] != ' '))
        i++; /* look for space after it */
    i++;
    if(i >= MAX_FIELD_LENGTH)
        return (0);
    for(j = 0; i < end; j++, i++)
        value[j] = str[i];
    value[j] = 0;
    if(value[0] == 's')
        return (MUS_NIST_SHORTPACK);
    sscanf(value, "%d", &i);
    return (i);
}
...
static int read_nist_header(const char* filename, int chan)
{
    ...
    while(happy)
    {
        ...
        str[k] = (char)hdrbuf[n];
        if((((str[k] == '\0') || (str[k] == '\n')) || ((curbase + n + 1) >= (off_t)data_location)) || (k == 79))
        {
            ...
            if(strcmp(name, "sample_rate") == 0)
                srate = decode_nist_value(str, nm, k);
            else if(strcmp(name, "channel_count") == 0)
                chans = (unsigned)decode_nist_value(str, nm, k);
            else if(strcmp(name, "end_head") == 0)
            {
                happy = false;
                comment_end = curbase + n - 9;
            } else if(strcmp(name, "sample_count") == 0)
                samples = decode_nist_value(str, nm, k);
            else if((bytes == 0) && (strcmp(name, "sample_n_bytes") == 0))
                bytes = decode_nist_value(str, nm, k);
            else if((bytes == 0) && (strcmp(name, "sample_sig_bits") == 0))
            {
                bytes = decode_nist_value(str, nm, k);
                bytes = (bytes >> 3);
            } else if(strcmp(name, "sample_byte_format") == 0)
                byte_format = decode_nist_value(str, nm, k);
            for(j = 0; j <= k; j++)
                str[j] = ' ';
            k = 0;
            if((curbase + n + 1) > 1024)
...
static int write_nist_header(int chan, int wsrate, unsigned wchans, unsigned siz, int format)
{
    char* header;
    unsigned datum;
    ...
    if(!header)
        return MUS_MEMORY_ALLOCATION_FAILED;
    sprintf(header,
            "NIST_1A\n   1024\nchannel_count -i %d\nsample_rate -i %d\nsample_n_bytes -i %d\nsample_byte_format -s2 %s\nsample_sig_bits -i "
            "%d\nsample_count -i %d\nend_head\n",
            wchans, wsrate, datum, ((format == MUS_BSHORT) || (format == MUS_B24INT) || (format == MUS_BINT)) ? "10" : "01", datum * 8,
            siz / datum);
...
static int read_bicsf_header(const char* filename, int chan)
{
    ...
    while(happy)
    {
        if(((offset + chunkloc) >= (off_t)data_location) || ((offset + chunkloc) < 40))
            happy = false;
        else
        {
            offset += chunkloc;
...
static int read_ircam_header(const char* filename, int chan)
{
    ...
    bool little, happy;
    type_specifier = mus_char_to_uninterpreted_int((unsigned char*)hdrbuf);
    if((mus_char_to_lint((unsigned char*)hdrbuf) == I_IRCAM_VAX) || (mus_char_to_lint((unsigned char*)hdrbuf) == I_IRCAM_MIPS))
        little = true;
    else
        little = false;
    little_endian = little;
    ...
    } else if(original_data_format == 0x40004)
    {
        if(little)
            ...
            data_format = MUS_BINT;
    } else if(original_data_format == 0x10001)
        data_format = MUS_ALAW;
    else if(original_data_format == 0x20001)
        data_format = MUS_MULAW;
    else if(original_data_format == 1)
        data_format = MUS_BYTE;
    ...
    chans = big_or_little_endian_uint((unsigned char*)(hdrbuf + 8), little);
    bloc = 16;
    happy = true;
    offset = 0;
    while(happy)
    {
        ...
        bloc = bsize;
        if((bsize <= 0) || (bcode <= 0) || ((offset + bloc) > 1023))
            happy = false;
    }
    data_size = mus_bytes_to_samples(data_format, data_size);
    return (MUS_NO_ERROR);
...
static int sndlib_format_to_ircam(int format)
{
    switch(format)
    {
        ...
        case MUS_BFLOAT: return (4); break;
        default:
            return (mus_error(MUS_UNSUPPORTED_DATA_FORMAT, "IRCAM header unsupported data format: %d (%s)", format,
                              any_data_format_name(format)));
            break;
    }
}
...
static void write_ircam_comment(int fd, const char* comment, int len)
{
    if(len > 0)
    {
        ...
        CHK_WRITE(fd, (const unsigned char*)comment, len);
    } else
    {
...
static int write_ircam_header(int chan, int wsrate, unsigned wchans, int format, const char* comment, int len)
{
    mus_bint_to_char((unsigned char*)hdrbuf, 0x2a364); /* SUN id */
    mus_bfloat_to_char((unsigned char*)(hdrbuf + 4), (float)wsrate);
    mus_ubint_to_char((unsigned char*)(hdrbuf + 8), wchans);
...
{
    unsigned chunksize, offset, chunkloc;
    bool happy;
    type_specifier = mus_char_to_uninterpreted_int((unsigned char*)hdrbuf);
    chunkloc = 12;
    offset = 0;
    ...
    happy = true;
    true_file_length = SEEK_FILE_LENGTH(chan);
    update_form_size = mus_char_to_ubint((unsigned char*)(hdrbuf + 4));
    while(happy)
    {
        offset += chunkloc;
        if(seek_and_read(chan, (unsigned char*)hdrbuf, offset, 32) <= 0)
        ...
        {
            chans = mus_char_to_ubint((unsigned char*)(hdrbuf + 8));
...
static int read_voc_header(const char* filename, int chan)
{
    ...
    if(true_file_length < curbase)
        return (
          mus_error(MUS_HEADER_READ_FAILED, "%s: block location %d > file length: %d", filename, (int)curbase, (int)true_file_length));
        ...
        } else
        {
            if(type == 9) /* voc_data_16 */
            {
                ...
                if(bits == 8)
                {
                    ...
                    data_format = MUS_UNKNOWN;
                chans = (unsigned)hdrbuf[9];
                if(chans == 0)
                    chans = 1;
                happy = false;
            } else
            {
                if(((len + curbase) < (off_t)true_file_length) && (type != 0))
                {
                    if(type == 5) /* voc_text */
                    {
    ...
    if((data_size > true_file_length) || (data_size < (true_file_length / 10))) /* some VOC files seem to have completely bogus lengths */
    {
        data_size = true_file_length - data_location;
        if(data_size < 0)
            return (mus_error(MUS_HEADER_READ_FAILED, "%s: data_size = %d?", filename, (int)data_size));
...
static int read_twinvq_header(const char* filename, int chan)
{
    ...
    chans = 1 + mus_char_to_ubint((unsigned char*)(hdrbuf + 24));
...
static int read_nvf_header(const char* filename, int chan)
{
    ...
    */
    if(mus_char_to_lint((unsigned char*)(hdrbuf + 4)) != 1)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s: NVF[4] != 1", filename));
    if(!(match_four_chars((unsigned char*)(hdrbuf + 12), I_VFMT)))
        return (mus_error(MUS_HEADER_READ_FAILED, "%s: no VFMT chunk", filename));
...
static int read_adc_header(const char* filename, int chan)
{
    ...
    if(little)
        ...
        data_format = MUS_BSHORT;
    chans = (unsigned)big_or_little_endian_short((unsigned char*)(hdrbuf + 4), little);
    srate = 4000000 / big_or_little_endian_short((unsigned char*)(hdrbuf + 6), little);
    data_size = 2 * big_or_little_endian_uint((unsigned char*)(hdrbuf + 8), little);
    comment_start = 0;
    comment_end = 0;
    true_file_length = SEEK_FILE_LENGTH(chan);
    if(data_size > true_file_length)
 ...
 *  22: srate
 *      avr.txt has:
 *      22: Replay speed 0 = 5.485 Khz, 1 = 8.084 Khz, 2 = 10.971 Khz, 3 = 16.168 Khz, 4 = 21.942 Khz, 5 = 32.336 Khz, 6 = 43.885 Khz, 7
 * = 47.261 Khz 23: sample rate	in Hertz (as a 3 byte quantity??) 26: length in samples 30: loop beg, 34: loop end, 38: midi (keyboard
 * split), 40: compression, 42: nada ("reserved"), 44: name 64: comment (limited to 64 bytes) 128: data start
 *
...
{
    int dsize, dsigned, i;
    int tmpChans = mus_char_to_bshort((unsigned char*)(hdrbuf + 12));
    if(tmpChans == 0)
        chans = 1;
    else if(tmpChans == -1)
        chans = 2;
    else
        return (mus_error(MUS_HEADER_READ_FAILED, "%s chans: %d", filename, tmpChans));
    data_location = 128;
...
{
    if(hdrbuf[4] != 'D')
        return (mus_error(MUS_HEADER_READ_FAILED, "%s: SNDT[4] != 'D'", filename));
    data_format = MUS_UBYTE;
    chans = 1;
    srate = mus_char_to_ulshort((unsigned char*)(hdrbuf + 20));
    data_location = 126;
    data_size = mus_char_to_ulint((unsigned char*)(hdrbuf + 8));
    if((int)data_size < 0)
        data_size = mus_char_to_ulint((unsigned char*)(hdrbuf + 10));
    if(srate <= 1)
        srate = mus_char_to_ulshort((unsigned char*)(hdrbuf + 22));
    true_file_length = SEEK_FILE_LENGTH(chan);
    if(data_size > true_file_length)
    {
        data_size = true_file_length - data_location;
...
static int read_smp_header(const char* filename, int chan)
{
    ...
    if(read(chan, hdrbuf, 4) != 4)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s: SMP header truncated?", filename));
    data_size = (mus_char_to_ulint((unsigned char*)hdrbuf));
...
static int read_sppack_header(const char* filename, int chan)
{
    ...
    if(read(chan, hdrbuf, 22) != 22)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s SPPACK header truncated?", filename));
...
static int read_esps_header(const char* filename, int chan)
{
    ...
    off_t curbase, hend;
    int k, j, n;
    unsigned chars, floats, shorts, doubles, bytes;
    bool little;
    little = (hdrbuf[18] == 0);
    if(little)
        data_location = mus_char_to_ulint((unsigned char*)(hdrbuf + 8));
    else
        data_location = mus_char_to_ubint((unsigned char*)(hdrbuf + 8));
    true_file_length = SEEK_FILE_LENGTH(chan);
    data_size = true_file_length - data_location;
    if(data_size < 0)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s: data_size = %d?", filename, (int)data_size));
    ...
    if(little)
    {
        doubles = mus_char_to_ulint((unsigned char*)hdrbuf);
        floats = mus_char_to_ulint((unsigned char*)(hdrbuf + 4));
        shorts = mus_char_to_ulint((unsigned char*)(hdrbuf + 12));
        chars = mus_char_to_ulint((unsigned char*)(hdrbuf + 16));
    } else
    {
        doubles = mus_char_to_ubint((unsigned char*)hdrbuf);
        floats = mus_char_to_ubint((unsigned char*)(hdrbuf + 4));
        shorts = mus_char_to_ubint((unsigned char*)(hdrbuf + 12));
        chars = mus_char_to_ubint((unsigned char*)(hdrbuf + 16));
    }
    if(shorts != 0)
    {
        ...
        { /* 3 = C-C marks end of record (?) */
            ...
            if(strcmp(str, "record_freq") == 0)
            {
                ...
                if(little)
                    ...
                    srate = (int)mus_char_to_bdouble((unsigned char*)(hdrbuf + 8));
                happy = false;
            }
            if((curbase + n + 1) >= data_location)
                happy = false;
            k = 0;
        } else
            k++;
...
static int read_csl_header(const char* filename, int chan)
{
    ...
    chans = 1;
    happy = true;
    update_form_size = mus_char_to_ulint((unsigned char*)(hdrbuf + 8));
    while(happy)
    {
        offset += chunkloc;
        ...
        if((chunksize == 0) && /* can be empty data chunk? */
           (hdrbuf[0] == 0) && (hdrbuf[1] == 0) && (hdrbuf[2] == 0) && (hdrbuf[3] == 0))
            break;
        if((match_four_chars((unsigned char*)hdrbuf, I_HEDR)) || (match_four_chars((unsigned char*)hdrbuf, I_HDR8)))
        {
            /* 8..20: date as ascii */
        ...
        } else
        {
            ...
            {
                if((match_four_chars((unsigned char*)hdrbuf, I_SDA_)) || (match_four_chars((unsigned char*)hdrbuf, I_SDAB))
    ...
    while(i < 1024)
    {
        ...
        if(strncmp(locbuf + i, "msb", 3) == 0)
            if(i + 9 < 1024 && strncmp(locbuf + i + 4, "first", 5) == 0)
                data_format = MUS_BSHORT;
        while((i < 1024) && (locbuf[i] != 10) && (locbuf[i] != 0))
            i++;
        i++;
    }
    FREE(locbuf);
...
static int read_sd1_header(const char* filename, int chan)
{
    ...
    if(read(chan, hdrbuf, 64) != 64)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s Sound Designer I header truncated?", filename));
...
{
    if((hdrbuf[13] != '*') || (hdrbuf[14] != '*'))
        return (mus_error(MUS_HEADER_READ_FAILED, "%s: PSION[13, 14] != '*'", filename));
    chans = 1;
    data_location = 32;
    srate = 8000;
    data_format = MUS_ALAW;
...
static int read_gravis_header(const char* filename, int chan)
{
    ...
    if(read(chan, hdrbuf, 128) != 128)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s PAT header truncated?", filename));
    chans = hdrbuf[84];
    if(chans == 0)
        chans = 1;
    comment_start = 22;
...
static int read_sbstudio_header(const char* filename, int chan)
{
    ...
    while(happy)
    {
        ...
        } else
        {
            ...
            {
                tmp = (unsigned)mus_char_to_lshort((unsigned char*)(bp + 15));
                if((tmp & 1) == 0)
                    data_format = MUS_UNKNOWN;
                else
                {
            ...
            } else
            {
                ...
                {
                    tmp = mus_char_to_ulint((unsigned char*)(bp + 4));
                    i += tmp;
                    bp += tmp;
                } else
                {
...
{
    if((hdrbuf[4] != '5') || (hdrbuf[5] != '3'))
        return (mus_error(MUS_HEADER_READ_FAILED, "%s TX16 magic number bogus", filename));
    chans = 1;
    data_location = 32;
    true_file_length = SEEK_FILE_LENGTH(chan);
    data_size = (true_file_length - data_location);
    ...
    else if(hdrbuf[23] == 0)
    {
        if((hdrbuf[26] & 0xFE) == 6)
            srate = 33000;
        else if((hdrbuf[26] & 0xFE) == 0x10)
            srate = 50000;
        else if((hdrbuf[26] & 0xFE) == 0xf6)
            srate = 16000;
    }
...
{
    if((hdrbuf[4] != ' ') && (hdrbuf[4] != 'A'))
        return (mus_error(MUS_HEADER_READ_FAILED, "%s: unknown magic number", filename));
    chans = 1;
    data_location = 1024;
    true_file_length = SEEK_FILE_LENGTH(chan);
    data_size = (true_file_length - data_location);
...
static int read_maui_header(const char* filename, int chan)
{
    ...
    if(read(chan, hdrbuf, 64) != 64)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s truncated maui header?", filename));
    chans = 1;
    data_location = 776;
    true_file_length = SEEK_FILE_LENGTH(chan);
    if(true_file_length < data_location)
...
static int read_akai3_header(const char *filename, int chan)
{
  ...
  true_file_length = SEEK_FILE_LENGTH(chan);
  data_size = (true_file_length - data_location);
  if (data_size < 0) return(mus_error(MUS_HEADER_READ_FAILED, "%s: data_size = %d?", filename, (int)data_size));
...
static int read_digiplayer_header(const char* filename, int chan)
{
    ...
    {
        if(hdrbuf[30] & 4)
            data_format = MUS_UBYTE; /* may be backwards -- using Convert 1.4 output here */
    }
    data_size = mus_bytes_to_samples(data_format, data_size);
    return (MUS_NO_ERROR);
}
...
static int read_adf_header(const char* filename, int chan)
{
    int bits, numsys;
    lseek(chan, 0, SEEK_SET);
    if((hdrbuf[4] != '4') || (hdrbuf[5] != '0'))
        return (mus_error(MUS_HEADER_READ_FAILED, "%s csre header bad magic number", filename));
    if(read(chan, hdrbuf, 30) != 30)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s csre header truncated?", filename));
    chans = 1;
...
static int read_diamondware_header(const char* filename, int chan)
{
    lseek(chan, 0, SEEK_SET);
    if(read(chan, hdrbuf, 64) != 64)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s truncated diamondware header?", filename));
    chans = hdrbuf[34];
    if(hdrbuf[31] == 0)
    {
        if(hdrbuf[35] == 8)
...
static int read_paf_header(const char* filename, int chan)
{
    ...
    bool little = false;
    lseek(chan, 0, SEEK_SET);
    if(read(chan, hdrbuf, 32) != 32)
        return (mus_error(MUS_HEADER_READ_FAILED, "%s PAF header truncated?", filename));
    data_format = MUS_UNKNOWN;
    if(mus_char_to_bint((unsigned char*)(hdrbuf + 8)))
        little = true;
    if(little)
    {
        srate = mus_char_to_lint((unsigned char*)(hdrbuf + 12));
        form = mus_char_to_lint((unsigned char*)(hdrbuf + 16));
        if(form == 0)
            data_format = MUS_LSHORT;
        if(form == 2)
            data_format = MUS_BYTE;
    ...
    {
        srate = mus_char_to_bint((unsigned char*)(hdrbuf + 12));
        form = mus_char_to_bint((unsigned char*)(hdrbuf + 16));
        if(form == 0)
            data_format = MUS_BSHORT;
        if(form == 2)
            data_format = MUS_BYTE;
...
static int read_comdisco_header(const char* filename, int chan)
{
    ...
    while(happy)
    {
        for(i = 0; i < 256; i++)
        {
            ...
            line[i] = (char)hdrbuf[k++];
        }
        line[i] = '\0';
        if((strcmp(line, "$DATA BINARY") == 0) || (strcmp(line, "$DATA ASCII") == 0))
        {
            happy = false;
        ...
        } else
        {
            if(commenting)
            {
                if(line[0] == '$')
                {
                    comment_end = offset + k - 2 - (int)strlen(line);
                    commenting = false;
                }
            }
        }
        if(line[0] != '$')
        {
            ...
            for(n = 0, m = j + 2; m < len; m++, n++)
                value[n] = line[m];
            value[n] = '\0';
            if(strcmp(portion, "Sampling") == 0)
                sscanf(value, "%d", &srate);
            else if(strcmp(portion, "Number o") == 0)
                sscanf(value, "%d", &d_size);
            else if(strcmp(portion, "Signal T") == 0)
            {
                if(value[1] == 'o')
                    type = 2;
                else if(value[1] == 'l')
                    type = 1;
            } else if(strcmp(portion, "Fixed Po") == 0)
            {
                if(value[1] == '8')
                    type = 3;
            }
...
static int read_asf_header(const char* filename, int chan)
{
    ...
    while(i < len)
    {
        ...
        if((unsigned int)(hdrbuf[1]) == 0x29)
            switch(hdrbuf[0])
            {
                case 0xd0: asf_huge = (hdrbuf[((128 + 64 + 128 + 64 + 64 + 64 + 64 + 32) / 8)] & 2); break;
                case 0xd4:
                    present = ((hdrbuf[16 + 8 + 16 + 8 + 8 + 4 + 4 + 4 + 4 + 4 + 4] >> 3) & 0x3);
                    if(present)
                        j = 16 + 8 + 16 + 8 + 8 + 4 + 4 + 4 + 4 + 4 + 4 + 4 + (4 + 4 + 4) + 2;
                    else
                        j = 16 + 8 + 16 + 8 + 8 + 4 + 4 + 4 + 4 + 4 + 4 + 4 + 2;
...
static unsigned header_raw_chans = 2;
...
void mus_header_set_raw_defaults(int sr, int chn, int frm)
{
    if(sr > 0)
        header_raw_srate = sr;
    if(chn > 0)
        header_raw_chans = (unsigned)chn;
    if(MUS_DATA_FORMAT_OK(frm))
...
static int mus_header_read_1(const char* filename, int chan)
{
    ...
    {
        if((match_four_chars((unsigned char*)(hdrbuf + 4), I_D_SA)) && (match_four_chars((unsigned char*)(hdrbuf + 8), I_MPLE)))
        {
            header_type = MUS_SMP;
    ...
    if((match_four_chars((unsigned char*)hdrbuf, I_file)) && (match_four_chars((unsigned char*)(hdrbuf + 4), I__sam)))
    {
    ...
    if((match_four_chars((unsigned char*)hdrbuf, I_PVF1)) || (match_four_chars((unsigned char*)hdrbuf, I_PVF2)))
    {
        header_type = MUS_PVF;
    ...
    if((match_four_chars((unsigned char*)hdrbuf, I__PAF)) || (match_four_chars((unsigned char*)hdrbuf, I_FAP_)))
    {
        header_type = MUS_PAF;
    ...
    if((match_four_chars((unsigned char*)hdrbuf, I_ones)) && (match_four_chars((unsigned char*)(hdrbuf + 12), I_FORM)))
    {
    ...
    if((match_four_chars((unsigned char*)hdrbuf, I_zeros)) && (match_four_chars((unsigned char*)(hdrbuf + 4), I_mdat)))
    {
        /* possibly quicktime?? */
...
#endif
    if((match_four_chars((unsigned char*)hdrbuf, I_asf0)) && (match_four_chars((unsigned char*)(hdrbuf + 4), I_asf1))
       && (match_four_chars((unsigned char*)(hdrbuf + 8), I_asf2)) && (match_four_chars((unsigned char*)(hdrbuf + 12), I_asf3)))
    {
        header_type = MUS_ASF;
...
int mus_header_read(const char* name)
{
    ...
    CLOSE(chan, name);
    if(err != MUS_NO_ERROR)
        return (mus_error(local_error_type, "%s",
                          local_error_msg)); /* pass error info on up the chain now that we've cleaned up the open file descriptor */
    return (err);
}
                     ...
                     const char* comment, int len)
{
    ...
    switch(type)
    {
        ...
        case MUS_RAW:
            data_location = 0;
            data_size = mus_bytes_to_samples(format, (off_t)siz);
            srate = in_srate;
            chans = in_chans;
            header_type = MUS_RAW;
            data_format = format;
...
int mus_header_change_data_size(const char* filename, int type, off_t size) /* in bytes */
{
    ...
    switch(type)
    {
        ...
        case MUS_AIFC:
        case MUS_NIST:
        case MUS_RIFF: err = mus_header_read(filename); break;
        default: break;
    }
    if(err == MUS_ERROR)
        return (err);
    ...
    if(chan == -1)
        return (mus_error(MUS_HEADER_WRITE_FAILED, "%s: %s", filename, STRERROR(errno)));
    switch(type)
    {
        case MUS_NEXT:
        ...
        case MUS_AIFF:
            ...
            mus_bint_to_char((unsigned char*)hdrbuf,
                             (int)size + (int)update_form_size - (int)mus_samples_to_bytes(data_format, (size_t)data_size));
            /* cancel old data_size from previous possible write */
            CHK_WRITE(chan, hdrbuf, 4);
            lseek(chan, update_frames_location, SEEK_SET);
            mus_bint_to_char((unsigned char*)hdrbuf, (int)size / (int)(chans * mus_bytes_per_sample(data_format)));
            CHK_WRITE(chan, hdrbuf, 4);
        ...
        default:
            CLOSE(chan, filename);
            return (
              mus_error(MUS_UNSUPPORTED_HEADER_TYPE, "mus_header_change_data_size: can't update %s headers", mus_header_type_name(type)));
            break;
    }
    CLOSE(chan, filename);
    return (err);
...
int mus_header_change_chans(const char* filename, int type, unsigned new_chans)
{
    ...
    switch(type)
    {
        case MUS_NEXT:
            ...
            mus_ubint_to_char((unsigned char*)hdrbuf, new_chans);
            CHK_WRITE(fd, hdrbuf, 4);
            break;
        case MUS_IRCAM:
            lseek(fd, 8L, SEEK_SET);
            if(little_endian)
                mus_ulint_to_char((unsigned char*)hdrbuf, new_chans);
            else
                mus_ubint_to_char((unsigned char*)hdrbuf, new_chans);
            CHK_WRITE(fd, hdrbuf, 4);
            break;
        ...
        case MUS_AIFC:
            ...
            mus_ubshort_to_char((unsigned char*)hdrbuf, new_chans);
        ...
        case MUS_RIFF:
            lseek(fd, update_frames_location - 2, SEEK_SET);
            if(little_endian)
                mus_ulshort_to_char((unsigned char*)hdrbuf, new_chans);
            else
                mus_ubshort_to_char((unsigned char*)hdrbuf, new_chans);
    ...
    switch(type)
    {
        ...
        case MUS_IRCAM:
            ...
            if(little_endian)
                ...
                mus_bfloat_to_char((unsigned char*)hdrbuf, (float)new_srate);
            CHK_WRITE(fd, hdrbuf, 4);
            break;
        case MUS_NIST:
            lseek(fd, 0L, SEEK_SET);
    ...
    if(err == MUS_NO_ERROR)
    {
        if(header_type != new_type)
        {
            ...
            if(new_type != MUS_RAW)
            {
                ...
                mus_header_write(new_file, new_type, srate, chans, loc, (size_t)data_size, new_format, comment, len);
            } else
                mus_file_create(new_file);
            ifd = mus_file_open_read(filename);
            lseek(ifd, loc, SEEK_SET);
            ofd = mus_file_reopen_write(new_file);
            lseek(ofd, 0L, SEEK_END);
            buf = (char*)CALLOC(8192, sizeof(char));
            while((nbytes = read(ifd, buf, 8192)))
                CHK_WRITE(ofd, buf, nbytes);
            CLOSE(ifd, filename);
            CLOSE(ofd, new_file);
            FREE(buf);
            if(comment)
    ...
    switch(type)
    {
        ...
        case MUS_RIFF:
            ...
            if(little_endian)
                ...
                mus_bshort_to_char((unsigned char*)hdrbuf, sndlib_format_to_aiff_bits(new_format));
            CHK_WRITE(fd, hdrbuf, 2);
            lseek(fd, update_frames_location + 10, SEEK_SET);
            switch(new_format)
            {
                case MUS_MULAW: new_format = 7; break;
                case MUS_ALAW: new_format = 6; break;
                case MUS_UBYTE:
                case MUS_LSHORT:
                case MUS_L24INT:
                case MUS_LINT:
                case MUS_BSHORT:
                case MUS_B24INT:
                case MUS_BINT: new_format = 1; break;
                case MUS_LFLOAT:
                case MUS_LDOUBLE:
                case MUS_BFLOAT:
                case MUS_BDOUBLE: new_format = 3; break;
            }
            if(little_endian)
                mus_lshort_to_char((unsigned char*)hdrbuf, new_format);
            else
                mus_bshort_to_char((unsigned char*)hdrbuf, new_format);
            CHK_WRITE(fd, hdrbuf, 2);
            break;
    }
    CLOSE(fd, filename);
    ...
    if(err == MUS_NO_ERROR)
    {
        switch(type)
        {
            ...
            case MUS_NEXT:
                ...
                {
                    if((comment_start != comment_end) && ((int)(data_location - 24) >= (int)strlen(new_comment)))
                ...
                CLOSE(fd, filename);
                break;
            default: need_ripple = true; break;
        }
        if(need_ripple)
        {
            /* open temp, write header, copy data, replace original with temp */
            ...
            sprintf(new_file, "%s.tmp", filename);
            loc = mus_header_data_location();
            mus_header_write(new_file, header_type, srate, chans, loc, (size_t)data_size, data_format, new_comment,
                             (new_comment) ? strlen(new_comment) : 0);
            ifd = mus_file_open_read(filename);
            lseek(ifd, loc, SEEK_SET);
            ofd = mus_file_reopen_write(new_file);
            lseek(ofd, 0L, SEEK_END);
    ...
    switch(type)
    {
        ...
        case MUS_NIST:
            ...
            switch(format)
            {
                ...
                case MUS_LSHORT:
                case MUS_L24INT:
                case MUS_LINT: return (true); break;
        ...
        case MUS_AIFF:
            ...
            switch(format)
            {
                ...
                case MUS_B24INT:
                case MUS_BINT:
                case MUS_BYTE: return (true); break;
                default: return (false); break;
            }
            break;
        case MUS_RIFF:
            ...
            switch(format)
            {
                ...
                case MUS_L24INT:
                case MUS_LINT:
                case MUS_LDOUBLE: return (true); break;
                default: return (false); break;
            }
            break;
        case MUS_IRCAM:
            ...
            switch(format)
            {
                ...
                case MUS_BSHORT:
                case MUS_BINT:
                case MUS_BFLOAT: return (true); break;
                default: return (false); break;
            }
            break;
        case MUS_RAW: return (true); break;
        default: return (false); break;
    }
    return (false);
}
    ...
    switch(type)
    {
        case MUS_NEXT:
            switch(format)
            {
                ...
                case 8: return ("indirect"); break;
                case 9: return ("nested"); break;
                case 10: return ("dsp_core"); break;
                case 11: return ("dsp_data_8"); break;
                case 12: return ("dsp_data_16"); break;
                case 13: return ("dsp_data_24"); break;
                case 14: return ("dsp_data_32"); break;
                case 16: return ("display"); break;
                case 17: return ("mulaw_squelch"); break;
                case 18: return ("emphasized"); break;
                case 19: return ("compressed"); break;
                case 20: return ("compressed_emphasized"); break;
                case 21: return ("dsp_commands"); break;
                case 22: return ("dsp_commands_samples"); break;
                case 23: return ("adpcm_g721"); break;
                case 24: return ("adpcm_g722"); break;
                case 25: return ("adpcm_g723"); break;
                case 26: return ("adpcm_g723_5"); break;
                case 28: return ("aes"); break;
                case 29: return ("delat_mulaw_8"); break;
            }
            break;
        case MUS_AIFC: aifc_format[4] = 0;
#if MUS_LITTLE_ENDIAN
        ...
        case MUS_RIFF:
            switch(format)
            {
                case 2: return ("ADPCM"); break;
                case 4: return ("VSELP"); break;
                case 5: return ("IBM_CVSD"); break;
                case 0x10: return ("OKI_ADPCM"); break;
                case 0x11: return ("DVI_ADPCM"); break;
                case 0x12: return ("MediaSpace_ADPCM"); break;
                case 0x13: return ("Sierra_ADPCM"); break;
                case 0x14: return ("G723_ADPCM"); break;
                case 0x15: return ("DIGISTD"); break;
                case 0x16: return ("DIGIFIX"); break;
                case 0x17: return ("Dialogic ADPCM"); break;
                case 0x18: return ("Mediavision ADPCM"); break;
                case 0x19: return ("HP cu codec"); break;
                case 0x20: return ("Yamaha_ADPCM"); break;
                case 0x21: return ("SONARC"); break;
                case 0x22: return ("DSPGroup_TrueSpeech"); break;
                case 0x23: return ("EchoSC1"); break;
                case 0x24: return ("AudioFile_AF36"); break;
                case 0x25: return ("APTX"); break;
                case 0x26: return ("AudioFile_AF10"); break;
                case 0x27: return ("prosody 1612"); break;
                case 0x28: return ("lrc"); break;
                case 0x30: return ("Dolby_Ac2"); break;
                case 0x31: return ("GSM610"); break;
                case 0x32: return ("MSN audio codec"); break;
                case 0x33: return ("Antext_ADPCM"); break;
                case 0x34: return ("Control_res_vqlpc"); break;
                case 0x35: return ("DIGIREAL"); break;
                case 0x36: return ("DIGIADPCM"); break;
                case 0x37: return ("Control_res_cr10"); break;
                case 0x38: return ("NMS_VBXADPCM"); break;
                case 0x39: return ("oland rdac"); break;
                case 0x3a: return ("echo sc3"); break;
                case 0x3b: return ("Rockwell adpcm"); break;
                case 0x3c: return ("Rockwell digitalk codec"); break;
                case 0x3d: return ("Xebec"); break;
                case 0x40: return ("G721_ADPCM"); break;
                case 0x41: return ("G728 CELP"); break;
                case 0x42: return ("MS G723"); break;
                case 0x50: return ("MPEG"); break;
                case 0x52: return ("RT24"); break;
                case 0x53: return ("PAC"); break;
                case 0x55: return ("Mpeg layer 3"); break;
                case 0x59: return ("Lucent G723"); break;
                case 0x60: return ("Cirrus"); break;
                case 0x61: return ("ESS Tech pcm"); break;
                case 0x62: return ("voxware "); break;
                case 0x63: return ("canopus atrac"); break;
                case 0x64: return ("G726"); break;
                case 0x65: return ("G722"); break;
                case 0x66: return ("DSAT"); break;
                case 0x67: return ("DSAT display"); break;
                case 0x69: return ("voxware "); break;
                case 0x70: return ("voxware ac8 "); break;
                case 0x71: return ("voxware ac10 "); break;
                case 0x72: return ("voxware ac16"); break;
                case 0x73: return ("voxware ac20"); break;
                case 0x74: return ("voxware rt24"); break;
                case 0x75: return ("voxware rt29"); break;
                case 0x76: return ("voxware rt29hw"); break;
                case 0x77: return ("voxware vr12 "); break;
                case 0x78: return ("voxware vr18"); break;
                case 0x79: return ("voxware tq40"); break;
                case 0x80: return ("softsound"); break;
                case 0x81: return ("voxware tq60 "); break;
                case 0x82: return ("MS RT24"); break;
                case 0x83: return ("G729A"); break;
                case 0x84: return ("MVI_MVI2"); break;
                case 0x85: return ("DF G726"); break;
                case 0x86: return ("DF GSM610"); break;
                case 0x88: return ("isaudio"); break;
                case 0x89: return ("onlive"); break;
                case 0x91: return ("sbc24"); break;
                case 0x92: return ("dolby ac3 spdif"); break;
                case 0x97: return ("zyxel adpcm"); break;
                case 0x98: return ("philips lpcbb"); break;
                case 0x99: return ("packed"); break;
                case 0x100: return ("rhetorex adpcm"); break;
                case 0x101: return ("Irat"); break;
                case 0x102: return ("IBM_alaw?"); break;
                case 0x103: return ("IBM_ADPCM?"); break;
                case 0x111: return ("vivo G723"); break;
                case 0x112: return ("vivo siren"); break;
                case 0x123: return ("digital g273"); break;
                case 0x200: return ("Creative_ADPCM"); break;
                case 0x202: return ("Creative fastspeech 8"); break;
                case 0x203: return ("Creative fastspeech 10"); break;
                case 0x220: return ("quarterdeck"); break;
                case 0x300: return ("FM_TOWNS_SND"); break;
                case 0x400: return ("BTV digital"); break;
                case 0x680: return ("VME vmpcm"); break;
                case 0x1000: return ("OLIGSM"); break;
                case 0x1001: return ("OLIADPCM"); break;
                case 0x1002: return ("OLICELP"); break;
                case 0x1003: return ("OLISBC"); break;
                case 0x1004: return ("OLIOPR"); break;
                case 0x1100: return ("LH codec"); break;
                case 0x1400: return ("Norris"); break;
                case 0x1401: return ("isaudio"); break;
                case 0x1500: return ("Soundspace musicompression"); break;
                case 0x2000: return ("DVM"); break;
            }
            break;
    }
...
bool mus_header_no_header(const char* filename)
{
    ...
    if(bytes > 4)
        ok =
          ((match_four_chars((unsigned char*)hdrbuf, I_DSND)) || (match_four_chars((unsigned char*)hdrbuf, I_DECN))
           || (match_four_chars((unsigned char*)hdrbuf, I_FORM)) || (match_four_chars((unsigned char*)hdrbuf, I_RIFF))
           || (match_four_chars((unsigned char*)hdrbuf, I_RIFX)) || (equal_big_or_little_endian((unsigned char*)hdrbuf, I_IRCAM_VAX))
           || (equal_big_or_little_endian((unsigned char*)hdrbuf, I_IRCAM_SUN))
           || (equal_big_or_little_endian((unsigned char*)hdrbuf, I_IRCAM_MIPS))
           || (equal_big_or_little_endian((unsigned char*)hdrbuf, I_IRCAM_NEXT)) || (match_four_chars((unsigned char*)hdrbuf, I_NIST))
           || (match_four_chars((unsigned char*)hdrbuf, I_SOUN)) || (match_four_chars((unsigned char*)hdrbuf, I_VOC0))
           || (match_four_chars((unsigned char*)hdrbuf, I_AVR_)) || (mus_char_to_bshort((unsigned char*)hdrbuf) == 1336)
           || (match_four_chars((unsigned char*)hdrbuf, I_ALaw)) || (match_four_chars((unsigned char*)hdrbuf, I_GF1P))
           || (match_four_chars((unsigned char*)hdrbuf, I_DSIG)) || (match_four_chars((unsigned char*)hdrbuf, I_GOLD))
           || (match_four_chars((unsigned char*)hdrbuf, I_Diam)) || (match_four_chars((unsigned char*)hdrbuf, I_SRFS))
           || (match_four_chars((unsigned char*)hdrbuf, I_CSRE)) || (match_four_chars((unsigned char*)hdrbuf, I_file))
           || ((hdrbuf[0] == 0xf0) && (hdrbuf[1] == 0x7e) && (hdrbuf[3] == 0x01))
           || (equal_big_or_little_endian((unsigned char*)(hdrbuf + 16), 0x00006a1a)) || (match_four_chars((unsigned char*)hdrbuf, I_SPIB))
           || (match_four_chars((unsigned char*)hdrbuf, I_S___)) || (match_four_chars((unsigned char*)hdrbuf, I_riff))
           || (match_four_chars((unsigned char*)hdrbuf, I_PVF1)) || (match_four_chars((unsigned char*)hdrbuf, I_PVF2))
           || (match_four_chars((unsigned char*)hdrbuf, I_MThd)) || (match_four_chars((unsigned char*)hdrbuf, I_SND_))
           || (match_four_chars((unsigned char*)hdrbuf, I_FSMt)) || (match_four_chars((unsigned char*)hdrbuf, I_DDSF))
           || (match_four_chars((unsigned char*)hdrbuf, I_LM89)) || (match_four_chars((unsigned char*)hdrbuf, I_SY85))
           || (match_four_chars((unsigned char*)hdrbuf, I_SY80)) || (match_four_chars((unsigned char*)hdrbuf, I_PRAM))
           || (match_four_chars((unsigned char*)(hdrbuf + 35), I_UWFD)) || (match_four_chars((unsigned char*)(hdrbuf + 76), I_SCRS))
           || (match_four_chars((unsigned char*)hdrbuf, I_covox)) || (match_four_chars((unsigned char*)hdrbuf, I__PAF))
           || (match_four_chars((unsigned char*)hdrbuf, I_FAP_)) || (match_four_chars((unsigned char*)hdrbuf, I_TWIN))
           || (match_four_chars((unsigned char*)hdrbuf, I_IMPS)) || (match_four_chars((unsigned char*)hdrbuf, I_SMP1))
           || (match_four_chars((unsigned char*)hdrbuf, I_Maui)) || (match_four_chars((unsigned char*)hdrbuf, I_SDIF))
           || (match_four_chars((unsigned char*)hdrbuf, I_ajkg)) || (match_four_chars((unsigned char*)hdrbuf, I_NVF_)));
    return (!ok);
}
